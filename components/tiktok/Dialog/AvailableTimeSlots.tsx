...
interface Props {
  ...
  setTiktokMedia: Dispatch<SetStateAction<any>>;
}
...
async function createScheduledVideo(
  workspaceId: string,
  userId: string,
  draftId: string,
  date: string,
  ...
  return (
    <div className="h-10 px-3 w-full flex flex-row justify-between items-between border rounded-lg">
      ...
      <button
        ...
        className="text-[#363636] silka-medium text-sm hover:opacity-80"
      >
        Select Time
      </button>
    </div>
...
async function getAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}tiktok/read/availableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}tiktok/read/moreavailableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
export function TiktokAvailableTimeSlots({
  workspaceId,
  userId,
  draftId,
  isOpen,
