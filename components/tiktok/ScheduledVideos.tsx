...
import { TiktokScheduleTimesDialog } from './Dialog/ScheduleTimes';
import Dropdown from 'react-bootstrap/Dropdown';
import toast from 'react-hot-toast';
import { TiktokPickPost } from './Dialog/PickPost';
...
async function unscheduledVideo(
  scheduledId: string,
  workspaceId: string,
  userId: string
) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}tiktok/delete/scheduledvideo`,
      null,
      {
        params: {
...
async function getScheduledVideos(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}tiktok/read/scheduledvideos`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreVideos(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}tiktok/read/morescheduled`,
      {
        params: {
          workspaceId: workspaceId,
          cursor: cursor,
...
async function getThumbnail(fileId: string) {
  try {
    const result = await axios.get(
      `${apiUrl()}tiktok/read/getthumbnail`,
      {
        params: {
          fileId: fileId,
        },
        ...
        <div className="flex w-full flex-row justify-between items-between">
          <div className="flex flex-col justify-start items-start py-2 w-full">
            ...
            {value?.tweet?.fileId ? (
              <>
                {urlLoading ? (
                  <div className="mt-2 w-[90px] h-[160px] flex flex-col justify-center items-center rounded bg-gray-200">
                    <svg
                  ...
                  <img
                    className="w-[90px] h-[160px] rounded mt-2"
                    src={googleUrl}
                  />
                )}
              </>
            ...
            <p className="mt-3 text-xs silka-medium text-[#363636]">
              {value?.tweet?.text}
            </p>
          </div>
          {hovered ? (
            ...
            <div className="my-auto">
              ...
              >
                <DialogPrimitive.Trigger asChild>
                  <button className="my-auto py-1 px-3 rounded text-xs hover:bg-[#EAEAEA] hover:opacity-80 silka-semibold text-[#363636]">
                    Pick Video
                  </button>
...
export function ScheduledVideos({ workspaceId, userId }: Props) {
        ...
        <div className="flex flex-col space-y-5 md:space-y-0 md:flex-row md:justify-between md:items-between">
          <div className="flex flex-col">
            ...
            <p className="text-xs silka-regular text-gray-500">
              Upcoming posts scheduled to be posted on TikTok.
            </p>
          </div>
