...
async function getSharesAnalytic(
  workspaceId: string,
  shares: number
) {
  try {
    ...
    const result = await axios.get(`${apiUrl()}tiktok/read/shares`, {
      params: { workspaceId: workspaceId, shares: shares },
    });
    return result.data;
  } catch (e) {
    console.log(e);
...
export function SharesAnalytic({
  workspaceId,
  shares,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (shares != null) {
      if (isPremium) {
        ...
        getSharesAnalytic(workspaceId, shares).then((value) => {
          setPercentChange(value.percentChange.toFixed(2));
          let temp: any = [];
          for (let i = 0; i < value.data.length; i++) {
            temp.push(value.data[i].shares);
          }
          setChartData(temp);
          setIsLoading(false);
        });
        ...
        let change: any = parseInt(shares) / second_to_last;
        setSharesCount(shares);
        if (shares < second_to_last) change = -Math.abs(1 - change);
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(sharesCount, 2)}
          </p>
        )}
        {isLoading ? (
