...
export function TiktokLikesChart({
  timeInterval,
  workspaceId,
  likes,
  views,
  isPremium,
}: TiktokLikesChartProps) {
  ...
  const data = {
    labels: selectedFormat == 'Likes' ? likesData : viewsData,
    datasets: [
      {
        label: selectedFormat == 'Likes' ? 'Likes' : 'Views',
        data: selectedFormat == 'Likes' ? likesData : viewsData,
    ...
    if (likes != null && views != null) {
      if (isPremium) {
        ...
        if (selectedFormat == 'Likes') {
          getLikesData(workspaceId, timeInterval, likes).then(
            (value) => {
              let temp = [];
          ...
          getViewsData(workspaceId, timeInterval, views).then(
            (value) => {
              let temp = [];
              for (let i = 0; i < value.length; i++) {
                if (value[i] < 0) {
        ...
        setIsLoading(true);
        if (selectedFormat == 'Likes') {
          setLikesData(randomData(timeInterval, likes));
          setIsLoading(false);
        } else {
          setViewsData(randomData(timeInterval, views));
          setIsLoading(false);
        }
      }
    }
  }, [likes, views, timeInterval, selectedFormat]);
        ...
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              chartValue={selectedFormat == 'Likes' ? likes : views}
              chartData={
                selectedFormat == 'Likes' ? likesData : viewsData
              }
              labelName={
                selectedFormat == 'Likes' ? 'Likes' : 'Views'
              }
              timeInterval={timeInterval}
              chartType={'bar'}
            />
