...
async function getEngagementRate(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}tiktok/read/engagementratechart`,
      {
        params: {
...
async function getEngagements(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}tiktok/read/engagementschart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function TiktokEngagementChart({
  timeInterval,
  workspaceId,
  comments,
  likes,
  ...
  views,
  isPremium,
}: TiktokEngagementChartProps) {
  const [isLoading, setIsLoading] = useState(true);
  const [selectedFormat, setSelectedFormat] = useState('Engagements');
  const [engagementRateData, setEngagementRateData] =
    useState<any>(null);
  const [engagementsData, setEngagementsData] = useState<any>(null);
  ...
  useEffect(() => {
    if (
      ...
      if (isPremium) {
        setIsLoading(true);
        setEngagement(likes + shares + comments);
        setRate(
          views > 0
            ? formatLargeNumber(
        ...
        setRate(
          views > 0
            ? (((likes + shares + comments) / views) * 100).toFixed(2)
            : 0
        );
        if (selectedFormat == 'Engagements') {
          setEngagementsData(
            randomData(timeInterval, likes + shares + comments, 'all')
          );
          setIsLoading(false);
        } else {
  ...
  }, [comments, likes, shares, views, selectedFormat, timeInterval]);

  const data = {
    labels:
      selectedFormat == 'Engagements'
        ? engagementsData
        : engagementRateData,
    datasets: [
      {
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {selectedFormat == 'Engagements'
                  ? formatLargeNumber(engagement, 1)
                  : formatLargeNumber(rate, 1) + '%'}
              </p>
            )}
        ...
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              chartValue={
                selectedFormat == 'Engagements' ? engagement : rate
              }
              chartData={
                selectedFormat == 'Engagements'
                  ? engagementsData
              ...
              timeInterval={timeInterval}
              chartType={
                selectedFormat == 'Engagements' ? 'bar' : 'line'
              }
            />
          </DialogPrimitive.Root>
