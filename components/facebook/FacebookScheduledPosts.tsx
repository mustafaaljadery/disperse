...
import { MyTimezoneDialog } from '../twitter/Dialog/MyTimezone';
import { FacebookScheduleTimeDialog } from './FacebookScheduleTimeDialog';
import { FacebookPickPost } from './dialogs/PickPost';
...
async function getThumbnail(fileId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/thumbnail`,
      {
        params: {
          fileId: fileId,
        },
...
async function unscheduleText(
  ...
  try {
    ...
    toast.loading('Unscheduling Text...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });

    const result = await axios.post(
      `${apiUrl()}facebook/delete/scheduledtext`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Text successfully unscheduled!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
    ...
    toast.remove();
    toast.error('Error unscheduling text, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
async function unscheduleImage(
  scheduledId: string,
  workspaceId: string,
  userId: string
) {
  try {
    ...
    toast.loading('Unscheduling Image...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });

    const result = await axios.post(
      `${apiUrl()}facebook/delete/scheduledimage`,
      null,
      {
        params: {
          scheduledId: scheduledId,
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Image successfully unscheduled!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
    ...
    toast.remove();
    toast.error('Error unscheduling image, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
async function unscheduleVideo(
  scheduledId: string,
  workspaceId: string,
  userId: string
) {
  try {
    ...
    toast.loading('Unscheduling Video...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });

    const result = await axios.post(
      `${apiUrl()}facebook/delete/scheduledvideo`,
      null,
      {
        params: {
          scheduledId: scheduledId,
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Video successfully unscheduled!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
    ...
    toast.remove();
    toast.error('Error unscheduling video, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
async function unscheduleReel(
  ...
  try {
    ...
    toast.loading('Unscheduling Reel...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });

    const result = await axios.post(
      `${apiUrl()}facebook/delete/scheduledreel`,
      null,
      {
        params: {
          scheduledId: scheduledId,
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Reel successfully unscheduled!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
    ...
    toast.remove();
    toast.error('Error unscheduling reel, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
async function unscheduleSlideshow(
  scheduledId: string,
  workspaceId: string,
  userId: string
) {
  try {
    ...
    toast.loading('Unscheduling Slideshow...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });

    const result = await axios.post(
      `${apiUrl()}facebook/delete/scheduledslideshow`,
      null,
      {
        params: {
          scheduledId: scheduledId,
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Slideshow successfully unscheduled!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
    ...
    toast.remove();
    toast.error('Error unscheduling slideshow, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
async function getScheduledPosts(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/scheduledposts`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMorePosts(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/morescheduled`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
      ...
      {value.tweet ? (
        <div className="flex w-full flex-row justify-between items-between">
          <div className="flex flex-col justify-start items-start py-2 w-full">
            ...
            <p
              className={
                'mt-1.5 text-sm silka-regular ' +
                (value.tweet.text
                  ? 'text-gray-600'
                  : 'text-gray-400 italic')
              }
            >
              {value.tweet.text ? value.tweet.text : 'No text...'}
            </p>
          </div>
          {hovered ? (
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        if (!confirmDelete) {
                          ...
                          } else if (value.tweet.type == 'image') {
                            unscheduleImage(
                              value.tweet.id,
                              workspaceId,
                              userId
                          ...
                          } else if (value.tweet.type == 'video') {
                            unscheduleVideo(
                              value.tweet.id,
                              workspaceId,
                              userId
          ...
          {hovered ? (
            <div className="my-auto">
              <DialogPrimitive.Root
                open={selectPostOpen}
                onOpenChange={setSelectPostOpen}
              >
                <DialogPrimitive.Trigger asChild>
                  <button className="my-auto py-1 px-3 rounded text-xs hover:bg-[#E8EEF5] hover:opacity-80 silka-semibold text-[#0975E8]">
                    Pick Post
                  </button>
                </DialogPrimitive.Trigger>
                <FacebookPickPost
                  workspaceId={workspaceId}
                  setIsOpen={setSelectPostOpen}
...
export function FacebookScheduledPosts({ workspaceId }: Props) {
  const [isLoading, setIsLoading] = useState(true);
  const [changeScheduleOpen, setChangeScheduleOpen] = useState(false);
  const [refetchScheduledPosts, setRefetchScheduledPosts] =
    useState(false);
  ...
  useEffect(() => {
    if (isVisible && !moreLoading) {
      ...
      getMorePosts(
        ...
        setCursor(cursor + 1);
        setScheduleData(scheduleData.concat(value.scheduled_posts));
  ...
  useEffect(() => {
    if (status == 'authenticated') {
      getScheduledPosts(workspaceId, session.user.id).then(
        (value) => {
          setScheduleData(value.scheduled_posts);
  ...
  useEffect(() => {
    if (refetchScheduledPosts && status == 'authenticated') {
      getScheduledPosts(workspaceId, session?.user?.id).then(
        (value) => {
          setScheduleData(value.scheduled_posts);
          setTimezone(value.timezone);
          setRefetchScheduledPosts(false);
        }
      );
    }
      ...
      <div className="flex flex-col space-y-5 md:space-y-0 md:flex-row md:justify-between md:items-between">
        <div className="flex flex-col">
          ...
          <p className="text-xs silka-regular text-gray-500">
            Upcoming posts scheduled to be posted on Facebook.
          </p>
        </div>
        <div className="flex flex-row space-x-4">
            ...
            <div className="my-auto">
              ...
              >
                ...
                <MyTimezoneDialog
                  ...
                  setRefetchScheduled={setRefetchScheduledPosts}
                />
              </DialogPrimitive.Root>
            </div>
          )}
          <div className="my-auto">
            ...
            >
              ...
              <FacebookScheduleTimeDialog
                isOpen={changeScheduleOpen}
                workspaceId={workspaceId}
                refetchScheduledPosts={refetchScheduledPosts}
                setRefetchScheduledPosts={setRefetchScheduledPosts}
                userId={String(session?.user?.id)}
              />
            </DialogPrimitive.Root>
          </div>
          ...
          {scheduleData?.map((value: any, index: number) => {
            return (
              <ScheduleDate
                ...
                value={value}
                key={index}
                setRefetchPosts={setRefetchScheduledPosts}
                userId={String(session?.user?.id)}
              />
            );
          })}
