...
import { FacebookImageOptions } from '../options/ImageOptions';
...
async function createDraftImage(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/create/draftimage`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/imagedraftdetails`,
      {
...
async function updateDraftImage(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/update/draftimage`,
      null,
      {
        params: {
          draftId: draftId,
      ...
      {imageMedia ? (
        ...
        >
          ...
          <p className="text-[10px] silka-regular text-gray-400">
            Pick an image to post on Facebook
          </p>
        </button>
      )}
      <textarea
