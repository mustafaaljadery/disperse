...
import { FacebookVideoOptions } from '../options/VideoOptions';
...
async function createDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/create/draftvideo`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/videodraftdetails`,
      {
...
async function updateDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/update/draftvideo`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    <div className="flex flex-col w-full">
      ...
      {videoMedia ? (
        ...
        <button
          ...
          className="mt-4 w-full flex flex-col space-y-2 justify-center items-center border rounded-xl border-dashed h-64 hover:border-[#0674E8]"
        >
          <p className="text-xs silka-medium">Select Media</p>
          <p className="text-[10px] silka-regular text-gray-400">
            Pick a video to post on Facebook
          </p>
        </button>
      )}
      <textarea
