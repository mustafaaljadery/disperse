...
import { FacebookSlideshowOptions } from '../options/SlideshowOptions';
...
async function createDraftSlideshow(
  workspaceId: string,
  text: string,
  fileId: any
) {
  try {
    ...
    fileId = fileId ? fileId[0].id : null;
    const result = await axios.post(
      `${apiUrl()}facebook/create/draftslideshow`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/slideshowdraftdetails`,
      {
        params: {
          draftId: draftId,
        },
...
async function updateDraftSlideshow(
  draftId: string,
  text: string,
  workspaceId: string
) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/update/draftslideshow`,
      null,
      {
        params: {
          draftId: draftId,
...
async function updateImages(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/update/slideshowimages`,
      null,
      {
        params: {
          draftId: draftId,
...
export function SlideshowLeft({
  ...
  useEffect(() => {
    if (selectedSlideshow) {
      getDraftDetails(selectedSlideshow).then((value) => {
        setText(value.text);
        setSlideshowMedia(value.images_data);
        if (value.images_data.length > 0) {
          setSelectedSlideshowIndex(0);
        } else {
    ...
    const timer = setTimeout(() => {
      if (text.length > 0) {
        if (selectedSlideshow) {
          ...
          createDraftSlideshow(
            workspaceId,
            text,
            slideshowMedia
          ).then((value) => {
            setSelectedSlideshow(value.id);
            setRefetchSlideshowDrafts(true);
            setIsSaving(false);
          });
        }
  ...
  useEffect(() => {
    if (slideshowMedia?.length > 0) {
      if (selectedSlideshow) {
        ...
        createDraftSlideshow(workspaceId, text, slideshowMedia).then(
          (value) => {
            setSelectedSlideshow(value.id);
            updateImages(value.id, slideshowMedia, workspaceId).then(
              (v) => {
                setRefetchSlideshowDrafts(true);
                setIsSaving(false);
              }
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="flex flex-row">
        <div className="my-auto w-[12.5%] flex flex-col justify-center items-center">
          <button
            onClick={(e) => {
              e.preventDefault();
              if (selectedSlideshowIndex > 0) {
                setSelectedSlideshowIndex(selectedSlideshowIndex - 1);
              }
            }}
        ...
        {selectedSlideshowIndex != null && selectedSlideshow ? (
          <img
            src={slideshowMedia[selectedSlideshowIndex].google_url}
          ...
          <button
            onClick={(e) => {
              e.preventDefault();
              setRightSlideshowSelected('Library');
            }}
        ...
        <div className="w-[12.5%] my-auto flex flex-col justify-center items-center rounded-full">
          <button
            onClick={(e) => {
              e.preventDefault();
              if (selectedSlideshowIndex < slideshowMedia.length - 1)
                setSelectedSlideshowIndex(selectedSlideshowIndex + 1);
            }}
            className="p-1.5 bg-[#E7EEF5] hover:bg-[#D8E5F3] rounded-full"
          >
      ...
      <div className="h-20 rounded-lg flex flex-row space-x-2 justify-start items-center px-3 w-full border border-dashed mt-3">
        {slideshowMedia?.map((value: any, index: number) => {
          return (
            <button
              key={index}
              onClick={(e) => {
        ...
        {slideshowMedia?.length < 20 && (
          <button
            ...
            className="h-16 rounded w-24 border border-dashed flex flex-col justify-center items-center hover:border-[#0674E8]"
          >
      ...
      <FacebookPostFooter
        ...
        comments={comments}
      />
      <FacebookSlideshowOptions
        workspaceId={workspaceId}
        draftId={selectedSlideshow}
        postText={text}
