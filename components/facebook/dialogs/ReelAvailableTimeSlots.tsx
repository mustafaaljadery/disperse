...
async function createScheduledReel(
  workspaceId: string,
  userId: string,
  draftId: string,
  date: string,
  ...
  try {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Successfully Scheduled Reel!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
    ...
    toast.remove();
    toast.error('Successfully Scheduled Reel!', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
export function FacbeookReelAvailableTimeSlots({
  workspaceId,
  userId,
  draftId,
  isOpen,
