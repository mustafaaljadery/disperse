...
async function createScheduledSlideshow(
  workspaceId: string,
  userId: string,
  draftId: string,
  date: string,
  ...
  try {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Successfully Scheduled Slideshow!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
    ...
    toast.remove();
    toast.error('Successfully Scheduled Slideshow!', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
  ...
  return (
    <div className="h-10 px-3 w-full flex flex-row justify-between items-between border rounded-lg">
      ...
      <button
        ...
        className="text-[#066CE5] silka-medium text-sm hover:opacity-80"
      >
        Select Time
      </button>
    </div>
...
async function getAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}facebook/read/availableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}facebook/read/moreavailableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
export function FacebookSlideshowAvailableTimeSlots({
  workspaceId,
  userId,
  draftId,
  isOpen,
