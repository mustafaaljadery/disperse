...
async function nextTimeSlot(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/create/textnexttimeslot`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Successfully Scheduled Text!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
      if (result.data.reason == 'slots') {
        ...
        toast.error('Error scheduling text, please try again.', {
          className: 'text-sm silka-medium text-gray-90',
        });
      }
    }
    return result.data;
...
export function FacebookScheduleText({
  isOpen,
  setIsOpen,
  workspaceId,
  autoRepostOpen,
          ...
          >
            <div className="w-full flex flex-col justify-center items-center">
              ...
              <p className="mt-2.5 text-xs silka-regular text-gray-400">
                Schedule text to be posted on Facebook.
              </p>
            </div>
