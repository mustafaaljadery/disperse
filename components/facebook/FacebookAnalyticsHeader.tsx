...
import { FacebookProfileComponent } from './FacebookProfileComponent';
import * as DialogPrimitive from '@radix-ui/react-dialog';
import { useSession } from 'next-auth/react';
import { ProUpgradeDialog } from '../../layouts/upgrade/ProUpgradeDialog';

interface FacebookAnalyticsHeaderProps {
  workspaceId: string;
...
async function getPageData(workspaceId: string) {
  try {
...
export function FacebookAnalyticsHeader({
  workspaceId,
  timeIncrement,
  setTimeIncrement,
  isPremium,
}: FacebookAnalyticsHeaderProps) {
  ...
  useEffect(() => {
    getPageData(workspaceId).then((value) => {
      setPageInfo(value);
      setIsLoading(false);
    });
  }, []);
      ...
      {!isPremium && (
        <div className="w-full py-6 border border-gray-200 rounded-lg flex flex-col justify-center items-center space-y-5">
          ...
          <p className="px-4 sm:px-6 md:px-10 lg:px-16 xl:px-32 text-sm silka-regular text-gray-600 text-center">
            Upgrade to pro to view all of your Facebook{' '}
          ...
          <div>
            ...
            >
              <DialogPrimitive.Trigger asChild>
                <button className="flex flex-row space-x-2 rounded hover:opacity-80 py-1.5 px-6 bg-[#066CE5]">
                  <svg
                    width="15"
                    height="15"
                    viewBox="0 0 15 15"
