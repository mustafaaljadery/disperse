...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/create/duplicatedraftvideo`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    toast.remove();
    toast.error('Error duplicating draft, please try agian.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/delete/draftvideo`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    toast.remove();
    toast.error('Error deleting draft, please try agian.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedVideo == value.id ? 'border border-[#0674E8]' : '')
        }
      >
        {value.image == 'yes' ? (
...
export function FacebookVideoRight({
  workspaceId,
  selectedVideo,
  setSelectedVideo,
  videoMedia,
