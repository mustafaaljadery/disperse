...
async function getProjectContent(folderId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/imagemedia`,
...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/create/duplicatedraftslideshow`,
      null,
      {
        params: {
          draftId: draftId,
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/delete/draftslideshow`,
      null,
      {
        params: {
          draftId: draftId,
...
interface Props {
  ...
  rightSlideshowSelected: any;
  setRightSlideshowSelected: Dispatch<SetStateAction<any>>;
            ...
            return (
              <div className="flex flex-row flex-wrap">
                {mediaData
                  ...
                  .map((value: any, index: number) => {
                    const seconds: any = formatSeconds(
                      value?.duration
                    );
                    return (
                          ...
                          <div className="relative rounded-lg flex flex-col w-full h-full">
                            ...
                            {value?.format.includes('video') && (
                              <div className="absolute flex flex-col justify-end items-end h-full p-1 w-full">
                                <p className="text-white px-2 py-1 text-[9px] silka-semibold bg-gray-900 rounded w-fit h-fit">
                                  {seconds}
                                </p>
                              </div>
                            )}
...
export function FacebookSlideshowRight({
  workspaceId,
  selectedSlideshow,
  setSelectedSlideshow,
  slideshowMedia,
  ...
  useEffect(() => {
    if (refetchSlideshowDrafts) {
      getDrafts(workspaceId).then((value) => {
        setDraftsData(value);
        setRefetchSlideshowDrafts(false);
      });
    }
  }, [refetchSlideshowDrafts]);
    ...
    <div className="flex flex-col">
      <div className="flex flex-row justify-between items-between">
        ...
        <button
          ...
          className={
            'text-xs px-4 py-1 silka-regular text-gray-800 rounded hover:bg-gray-100 ' +
            (rightSlideshowSelected == 'Library' ? 'bg-gray-100' : '')
          }
        >
          Library
        </button>
