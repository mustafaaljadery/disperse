...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/create/duplicatedraftimage`,
      null,
      {
        params: {
          draftId: draftId,
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}facebook/delete/draftimage`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedImage == value.id ? 'border border-[#066CE5]' : '')
        }
      >
        {value.image == 'yes' ? (
...
function Drafts({
  ...
  return (
    <div className="flex flex-row w-full mt-4 flex-wrap">
      {draftsData?.length > 0 ? (
        draftsData
...
function Media({ isLoading, mediaData, setImageMedia }: MediaProps) {
  if (isLoading) {
    return (
      <>
        <div className="flex flex-row space-x-3">
    ...
    <div className="mt-6 w-full flex flex-row space-x-7 flex-wrap">
      {projectsData.length == 0 ? (
        <div className="flex flex-col justify-center items-center w-full">
          <div className="p-2 bg-[#E8EFF5] rounded-full mt-4">
            <svg
              width="20"
              height="20"
              viewBox="0 0 24 24"
          ...
          <h3 className="text-sm silka-semibold text-gray-900 mt-3.5">
            No Media Found
          </h3>
...
export function FacebookImageRight({
  workspaceId,
  selectedImage,
  setSelectedImage,
  imageMedia,
  ...
  useEffect(() => {
    if (refetchImageDrafts) {
      getDrafts(workspaceId).then((value) => {
        setDraftsData(value);
        setRefetchImageDrafts(false);
      });
    }
  }, [refetchImageDrafts]);
    ...
    <div className="flex flex-col">
      <div className="flex flex-row justify-between items-between">
        ...
        <button
          ...
          className={
            'text-xs px-4 py-1 silka-regular text-gray-800 rounded hover:bg-gray-100 ' +
            (rightImageSelected == 'Library' ? 'bg-gray-100' : '')
          }
        >
          Library
        </button>
      </div>
      {rightImageSelected == 'Drafts' ? (
        <Drafts
          isLoading={isLoading}
