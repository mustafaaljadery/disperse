...
async function getPageEngagementData(
  workspaceId: string,
  timeInterval: string,
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/pageengagement`,
      {
        params: {
...
async function getUserEngagementData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/userengagement`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
async function getEngagementRateData(
  workspaceId: string,
  timeInterval: string,
  impressions: number,
  pageEngagement: number
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/engagementrate`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function FacebookEngagementsChart({
  workspaceId,
  timeInterval,
  impressions,
  pageEngagement,
  ...
  const [selectedFormat, setSelectedFormat] = useState('Page');
  ...
  useEffect(() => {
    if (
      ...
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Page') {
          getPageEngagementData(
            workspaceId,
            timeInterval,
            pageEngagement
          ).then((value) => {
            ...
            setPageEngagementData(temp);
            setIsLoading(false);
          });
        } else if (selectedFormat == 'User') {
        ...
        if (selectedFormat == 'Page') {
          setPageEngagementData(
            randomData(timeInterval, pageEngagement, 'page')
          );
          setIsLoading(false);
        } else if (selectedFormat == 'User') {
          setUserEngagementData(
            randomData(timeInterval, userEngagement, 'user')
          );
          setIsLoading(false);
        } else {
          setEngagementRateData(
            randomData(
              timeInterval,
              (pageEngagement / impressions) * 100,
              'rate'
            )
          );
          setIsLoading(false);
