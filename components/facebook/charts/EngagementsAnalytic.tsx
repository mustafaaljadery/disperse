...
async function getEngagementsAnalytic(
  workspaceId: string,
  engagements: number
) {
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}facebook/read/engagementsanalytic`,
      {
        params: {
...
export function FacebookEngagementsAnalytic({
  workspaceId,
  engagements,
  isPremium,
}: Props) {
  const [isLoading, setIsLoading] = useState(true);
  const [engagementsCount, setEngagementsCount] = useState(0);
  ...
  useEffect(() => {
    if (engagements != null) {
      if (isPremium) {
        ...
        getEngagementsAnalytic(workspaceId, engagements).then(
          (value) => {
            setPercentChange(value.percentChange.toFixed(2));
            let temp: any = [];
            for (let i = 0; i < value.data.length; i++) {
              temp.push(value.data[i].engagements);
            }
            setChartData(temp);
            setIsLoading(false);
          }
        ...
        let change: any = parseInt(engagements) / second_to_last;
        setEngagementsCount(engagements);
        if (engagements < second_to_last)
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(engagementsCount, 1)}
          </p>
        )}
        {isLoading ? (
