...
async function getPositiveReactions(
  workspaceId: string,
  timeInterval: string,
  positiveReactions: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/positivereactions`,
      {
        params: {
...
async function getNegativeReactions(
  workspaceId: string,
  timeInterval: string,
  negativeReactions: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/negativereactions`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function FacebookReactionsChart({
  workspaceId,
  timeInterval,
  positiveReactions,
  negativeReactions,
  ...
  const [positiveReactionsData, setPositiveReactionsData] =
    useState<any>(null);
  const [negativeReactionsData, setNegativeReactionsData] =
    useState<any>(null);
  const [selectedFormat, setSelectedFormat] = useState('Positive');
  const [screenshot, setScreenshot] = useState(false);

  useEffect(() => {
    if (positiveReactions != null && negativeReactions != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Positive') {
        ...
        setIsLoading(true);
        if (selectedFormat == 'Positive') {
          setPositiveReactionsData(
            randomData(timeInterval, positiveReactions)
          );
          setIsLoading(false);
        } else {
          setNegativeReactionsData(
            randomData(timeInterval, negativeReactions)
          );
          setIsLoading(false);
        }
      }
  ...
  const data = {
    labels:
      selectedFormat == 'Positive'
        ? positiveReactionsData
        : negativeReactionsData,
    datasets: [
      {
        label: 'Reactions',
        data:
      ...
      <div className="flex flex-row justify-between items-between">
        ...
        <div className="flex flex-row space-x-3">
          ...
          <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() => {
                setSelectedFormat('Negative');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Negative'
                  ? 'bg-white shadow text-gray-800'
                  : 'text-gray-500')
              }
            >
