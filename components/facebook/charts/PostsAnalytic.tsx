...
async function getPostsAnalytic(workspaceId: string, videos: number) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}facebook/read/posts`, {
...
export function FacebookPostsAnalytic({
  workspaceId,
  posts,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (posts != null) {
      if (isPremium) {
        setPostsCount(posts);
        getPostsAnalytic(workspaceId, posts).then((value) => {
          setPostsCount(value.data[value.data.length - 1]);
        ...
        let change: any = posts / second_to_last;
        setPostsCount(posts);
        if (posts < second_to_last) change = -Math.abs(1 - change);
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(postsCount, 1)}
          </p>
        )}
        {isLoading ? (
