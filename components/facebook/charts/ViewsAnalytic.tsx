...
async function getViewsAnalytic(workspaceId: string, views: number) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}facebook/read/views`, {
      params: { workspaceId: workspaceId, views: views },
    });
    return result.data;
  } catch (e) {
    console.log(e);
...
export function FacebookViewsAnalytic({
  workspaceId,
  views,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (views != null) {
      if (isPremium) {
        setViewsCount(views);
        getViewsAnalytic(workspaceId, views).then((value) => {
          setPercentChange(value.percentChange.toFixed(2));
          let temp: any = [];
          for (let i = 0; i < value.data.length; i++) {
            temp.push(value.data[i].views);
          }
          setChartData(temp);
          setIsLoading(false);
        });
