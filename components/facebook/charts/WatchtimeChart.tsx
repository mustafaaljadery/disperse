...
async function getTotalViewtimeData(
  workspaceId: string,
  timeInterval: string,
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/totalviewtime`,
      {
        params: {
...
export function FacebookWatchtimeChart({
  workspaceId,
  timeInterval,
  totalViewtime,
  averageViewtime,
  ...
  const [totalViewtimeData, setTotalViewtimeData] =
    useState<any>(null);
  const [selectedFormat, setSelectedFormat] = useState('Total');
  const [screenshot, setScreenshot] = useState(false);

  useEffect(() => {
    if (totalViewtime != null) {
      if (isPremium) {
        setIsLoading(true);
        getTotalViewtimeData(
          workspaceId,
          timeInterval,
          totalViewtime / 1000
        ).then((data) => {
          let temp: any = data.split(',');
          // all values to int
          temp = temp.map((x: any) => parseInt(x));
          let temp2 = [];
          ...
          setTotalViewtimeData(temp2);
          setIsLoading(false);
        });
      } else {
        setIsLoading(true);
        setTotalViewtimeData(randomData(timeInterval, totalViewtime));
        setIsLoading(false);
      }
    }
  }, [selectedFormat, timeInterval, totalViewtime, averageViewtime]);

  const data = {
    labels: totalViewtimeData,
    datasets: [
      {
        label: 'Watchtime Seconds',
        data: totalViewtimeData,
      ...
      <div className="flex flex-row justify-between items-between">
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(
                  totalViewtimeData[totalViewtimeData.length - 1],
                  1
                )}
              </p>
            )}
        ...
        >
          ...
          <DownloadChart
            ...
            chartName={'Facebook WatchTime (seconds)'}
            chartValue={
              isLoading
                ? 0
                : totalViewtimeData[totalViewtimeData.length - 1]
            }
            chartData={totalViewtimeData}
            labelName={'watchtime'}
            timeInterval={timeInterval}
            chartType={'bar'}
          />
        ...
        <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
          ...
          <button
            onClick={() => {
              setSelectedFormat('Average');
            }}
            className={
              'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
              (selectedFormat == 'Average'
                ? 'bg-white shadow text-gray-800'
                : 'text-gray-500')
            }
          >
