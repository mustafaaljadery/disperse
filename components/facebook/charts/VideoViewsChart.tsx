...
async function getTotalVideoViews(
  workspaceId: string,
  timeInterval: string,
  views: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/totalvideoviews`,
      {
        params: {
...
async function getUniqueVideoViews(
  workspaceId: string,
  timeInterval: string,
  uniqueViews: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/uniquevideoviews`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function FacebookVideoViewsChart({
  workspaceId,
  timeInterval,
  views,
  uniqueViews,
  ...
  const [totalViewsData, setTotalViewsData] = useState<any>(null);
  const [uniqueViewsData, setUniqueViewsData] = useState<any>(null);
  const [selectedFormat, setSelectedFormat] = useState('Total');
  const [videoViews, setVideoViews] = useState(0);
  const [screenshot, setScreenshot] = useState(false);

  useEffect(() => {
    if (views != null && uniqueViews != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Total') {
          getTotalVideoViews(workspaceId, timeInterval, views).then(
            (value) => {
              setVideoViews(value[value.length - 1]);
              let temp = [];
        ...
        setIsLoading(true);
        if (selectedFormat == 'Total') {
          setTotalViewsData(randomData(timeInterval, views));
          setIsLoading(false);
        } else {
          setUniqueViewsData(randomData(timeInterval, uniqueViews));
          setIsLoading(false);
        }
      }
    }
  }, [selectedFormat, timeInterval, uniqueViews, views]);

  const data = {
    labels:
      selectedFormat == 'Total' ? totalViewsData : uniqueViewsData,
    datasets: [
      {
        label: 'Video Views',
        data:
        ...
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              chartValue={
                selectedFormat == 'Total' ? videoViews : uniqueViews
              }
              chartData={
                selectedFormat == 'Total'
                  ? totalViewsData
          ...
          <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() => {
                setSelectedFormat('Unique');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Unique'
                  ? 'bg-white shadow text-gray-800'
                  : 'text-gray-500')
              }
            >
