...
async function getTotalImpressions(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/impressionstotal`,
      {
        params: {
...
async function getDailyImpressions(
  workspaceId: string,
  timeInterval: string,
  impressions: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/impressionsdaily`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function FacebookImpressionsChart({
  timeInterval,
  workspaceId,
  impressions,
  isPremium,
  ...
  const [dailyImpressionsData, setDailyImpressionsData] =
    useState<any>(null);
  const [totalImpressionsData, setTotalImpressionsData] =
    useState<any>(null);
  ...
  useEffect(() => {
    if (impressions != null) {
      if (isPremium) {
        ...
        setIsLoading(true);
        if (selectedFormat == 'Daily') {
          setDailyImpressionsData(
            randomData(timeInterval, impressions)
          );
          setIsLoading(false);
        } else {
  ...
  }, [selectedFormat, timeInterval, impressions]);
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(impressions, 1)}
              </p>
            )}
          </div>
        </div>
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              chartName={'Facebook Impressions'}
              chartValue={impressions}
              chartData={
                selectedFormat == 'Total'
                  ? totalImpressionsData
                  : dailyImpressionsData
              }
              labelName={'Impressions'}
              timeInterval={timeInterval}
