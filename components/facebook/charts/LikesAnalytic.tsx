...
async function getLikesAnalytic(workspaceId: string, likes: number) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}facebook/read/likes`, {
...
export function FacebookLikesAnalytic({
  workspaceId,
  likes,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (likes != null) {
      if (isPremium) {
        setLikesCount(likes);
        getLikesAnalytic(workspaceId, likes).then((value) => {
          setPercentChange(value.percentChange.toFixed(2));
          let temp: any = [];
          for (let i = 0; i < value.data.length; i++) {
            temp.push(value.data[i].likes);
          }
          setChartData(temp);
          setIsLoading(false);
        });
