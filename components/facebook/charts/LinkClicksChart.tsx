...
async function getLinkClicksData(
  workspaceId: string,
  timeInterval: string,
  linkClicks: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}facebook/read/linkclickschart`,
      {
        params: {
...
export function FacebookLinkClicksChart({
  workspaceId,
  timeInterval,
  linkClicks,
  isPremium,
  ...
  const [linkClicksChart, setLinkClicksChart] = useState<any>(null);
  ...
  useEffect(() => {
    if (linkClicks != null) {
      if (isPremium) {
        setIsLoading(true);
        getLinkClicksData(workspaceId, timeInterval, linkClicks).then(
          (value) => {
            let temp = [];
            ...
            setLinkClicksChart(temp);
            setIsLoading(false);
          }
        );
      } else {
        setLinkClicksChart(randomData(timeInterval, linkClicks));
        setIsLoading(false);
      }
    }
  }, [timeInterval, linkClicks]);

  const data = {
    labels: linkClicksChart,
    datasets: [
      {
        label: 'Link Clicks',
        data: linkClicksChart,
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(linkClicks, 1)}
              </p>
            )}
          </div>
        </div>
        <div className="my-auto">
          ...
          >
            ...
            <DownloadChart
              ...
              chartName={'Facebook Link Clicks'}
              chartValue={linkClicks}
              chartData={linkClicksChart}
              labelName={'Clicks'}
              timeInterval={timeInterval}
              chartType={'bar'}
            />
