...
async function getActiveWorkflows(workspaceId: string) {
  axiosRetry(axios, { retries: 3 });

  try {
    const result = await axios.get(
      `${apiUrl()}automation/read/activeautomations`,
      {
...
export function ActiveWorkflows({
  ...
  useEffect(() => {
    getActiveWorkflows(workspaceId).then((value) => {
  ...
  useEffect(() => {
    if (refetchActiveWorkflows) {
      setIsLoading(true);
      getActiveWorkflows(workspaceId).then((value) => {
        setWorkflowsData(value);
        setIsLoading(false);
        setRefetchActiveWorkflows(false);
      });
    }
  }, [refetchActiveWorkflows]);
