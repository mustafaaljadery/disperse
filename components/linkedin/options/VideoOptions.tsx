...
import { LinkedinPostVideoNow } from '../dialogs/PostVideoNow';
import { LinkedinScheduleVideo } from '../dialogs/ScheduleVideo';
import { useSession } from 'next-auth/react';
import { LinkedinVideoAvailableTimeSlot } from '../dialogs/VideoAvailableTimeSlot';
...
async function getOptions(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}linkedin/read/draftvideooptions`,
      {
...
async function updateOptions(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/update/draftvideooptions`,
      null,
      {
        params: {
          draftId: draftId,
...
export function VideoOptions({
  ...
  return (
    <div className="flex flex-col mt-6">
      ...
      >
        <LinkedinVideoAvailableTimeSlot
          workspaceId={workspaceId}
          userId={String(session?.user?.id)}
          draftId={draftId}
