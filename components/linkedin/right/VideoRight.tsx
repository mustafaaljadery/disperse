...
async function getProjectContent(folderId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}linkedin/read/videomedia`,
...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/create/duplicatedraftvideo`,
      null,
      {
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/delete/draftvideo`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedVideo == value.id ? 'border border-[#0966C2]' : '')
        }
      >
        {value.image == 'yes' ? (
...
export function LinkedinVideoRight({
  workspaceId,
  selectedVideo,
  setSelectedVideo,
  videoMedia,
