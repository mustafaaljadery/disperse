...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/create/duplicatedraftimage`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    toast.error('Error duplicating, please try again.', {
      className: 'text-gray-900 silka-medium text-sm',
    });
    console.log(e);
  }
}
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/delete/draftimage`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedImage == value.id ? 'border border-[#0966C2]' : '')
        }
      >
        {value.image == 'yes' ? (
...
export function LinkedinImageRight({
  workspaceId,
  selectedImage,
  setSelectedImage,
  imageMedia,
