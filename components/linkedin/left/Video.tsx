...
interface Props {
  ...
  setVideoRight: Dispatch<SetStateAction<any>>;
  isPremium: boolean;
  userId: string;
}
...
async function createDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/create/draftvideo`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}linkedin/read/videodraftdetails`,
      {
...
async function updateDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/update/draftvideo`,
      null,
      {
        params: {
          draftId: draftId,
...
export function LinkedinVideoLeft({
  workspaceId,
  selectedVideo,
  setSelectedVideo,
  videoMedia,
  ...
  const [comments, setComments] = useState<number>(between(5, 100));
  const [reposts, setReposts] = useState<number>(between(5, 100));
    ...
    const timer = setTimeout(() => {
      if (text.length > 0 || videoMedia) {
        if (selectedVideo) {
          ...
          createDraftVideo(workspaceId, text, videoMedia?.id).then(
            (value) => {
    ...
    <div className="flex flex-col w-full">
      ...
      {videoMedia ? (
        ...
        <button
          onClick={(e) => {
            e.preventDefault();
            setVideoRight('Library');
          }}
          className="mt-4 w-full flex flex-col space-y-2 justify-center items-center border rounded-xl border-dashed h-64 hover:border-[#0966C2]"
        >
          <p className="text-xs silka-medium">Select Media</p>
          <p className="text-[10px] silka-regular text-gray-400">
            Pick a video to post on Linkedin
          </p>
        </button>
      )}
      <textarea
        ...
        className="px-0 mt-2 text-sm silka-regular resize-none border-none text-gray-700 w-full focus:border-none focus:ring-0"
      />
      <LinkedinPostFooter
        comments={comments}
        reposts={reposts}
