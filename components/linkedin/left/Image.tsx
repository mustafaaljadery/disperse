...
interface Props {
  ...
  setImageRight: Dispatch<SetStateAction<any>>;
  isPremium: boolean;
  userId: string;
}
...
async function createDraftImage(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/create/draftimage`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}linkedin/read/imagedraftdetails`,
      {
...
async function updateDraftImage(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/update/draftimage`,
      null,
      {
        params: {
          draftId: draftId,
...
export function LinkedinImageLeft({
  workspaceId,
  selectedImage,
  setSelectedImage,
  imageMedia,
      ...
      {imageMedia ? (
        ...
        >
          ...
          <p className="text-[10px] silka-regular text-gray-400">
            Pick an image to post on Linkedin
          </p>
        </button>
      )}
      <textarea
