...
async function postMultiimageNow(
  workspaceId: string,
  draftId: string,
  setIsOpen: Dispatch<SetStateAction<boolean>>,
  ...
  try {
    ...
    toast.loading('Posting Multi-image...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });
    const result = await axios.post(
      `${apiUrl()}linkedin/create/postmultiimage`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      ...
      toast.remove();
      toast.error('Error posting multi-image, please try again.', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
  } catch (e) {
...
export function LinkedinPostMultiimageNow({
  isOpen,
  setIsOpen,
  removeUrlPreview,
  autoPlugOpen,
          ...
          >
            <div className="w-full flex flex-col justify-center items-center">
              ...
              <p className="mt-2.5 text-xs silka-regular text-gray-400">
                Post multi-image on Linkedin right now.
              </p>
            </div>
