...
async function nextTimeSlot(
  ...
  try {
    ...
    toast.loading('Scheduling Multi-image...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });
    const result = await axios.post(
      `${apiUrl()}linkedin/create/multiimagenexttimeslot`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Successfully Scheduled Multi-image!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
      if (result.data.reason == 'slots') {
        ...
        toast.error(
          'Error scheduling multi-image, please try again.',
          {
            className: 'text-sm silka-medium text-gray-900',
          }
        );
    ...
    toast.remove();
    toast.error('Error scheduling multi-image, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
...
export function LinkedinScheduleMultiimage({
  isOpen,
  setIsOpen,
  workspaceId,
  removeUrlPreview,
          ...
          >
            <div className="w-full flex flex-col justify-center items-center">
              <DialogPrimitive.Title className="text-2xl silka-bold text-gray-800">
                Schedule Multi-Image
              </DialogPrimitive.Title>
              <p className="mt-2.5 text-xs silka-regular text-gray-400">
                Schedule multi-image to be posted on Linkedin.
              </p>
            </div>
              ...
              <button
                onClick={(e) => {
                  ...
                  if (autoReshareOpen && reshareInputValue == 0) {
                    toast.error('Auto-Reshare Input Cannot be 0', {
                      className: 'text-sm silka-medium text-gray-900',
                    });
