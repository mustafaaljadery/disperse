...
async function PostVideoNow(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}linkedin/create/postvideo`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      ...
      toast.remove();
      toast.error('Error posting video, please try agian.', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
  } catch (e) {
...
export function LinkedinPostVideoNow({
  isOpen,
  setIsOpen,
  removeUrlPreview,
  autoPlugOpen,
          ...
          >
            <div className="w-full flex flex-col justify-center items-center">
              ...
              <p className="mt-2.5 text-xs silka-regular text-gray-400">
                Post video on Linkedin right now.
              </p>
            </div>
              ...
              <button
                onClick={(e) => {
                  ...
                  if (autoReshareOpen && reshareInputValue == 0) {
                    ...
                    toast.error(
                      'Please select an video to be posted.',
                      {
                        className:
