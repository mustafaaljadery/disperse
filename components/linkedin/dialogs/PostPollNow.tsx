...
async function postPollNow(
  ...
  try {
    ...
    toast.loading('Posting Poll...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });
    const result = await axios.post(
      `${apiUrl()}linkedin/create/postpoll`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      ...
      toast.remove();
      toast.success('Poll Successfully Posted!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
...
export function LinkedinPostPollNow({
  isOpen,
  setIsOpen,
  removeUrlPreview,
  autoPlugOpen,
          ...
          >
            <div className="w-full flex flex-col justify-center items-center">
              ...
              <p className="mt-2.5 text-xs silka-regular text-gray-400">
                Post poll on Linkedin right now.
              </p>
            </div>
            ...
            <DialogPrimitive.Close className="w-full">
              <button
                onClick={(e) => {
                  e.preventDefault();
                  setIsOpen(false);
                  let fieldsAccept = true;
                  for (let i = 0; i < pollFields.length; i++) {
                    if (pollFields[i].value === '') {
                      fieldsAccept = false;
                      break;
                    }
                  }
                  if (fieldsAccept == false) {
