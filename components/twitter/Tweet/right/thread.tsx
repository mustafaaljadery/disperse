...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}twitter/delete/draftthread`,
      null,
      {
        params: {
          draftId: draftId,
...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}twitter/create/duplicatedraftthread`,
      null,
      {
        params: {
          draftId: draftId,
  ...
  return (
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          'break-all w-full my-auto rounded-lg p-2.5 text-xs bg-gray-50 md:text-sm lg:text-xs xl:text-sm silka-regular text-start ' +
          (selectedThread == value.id
            ? ' border border-[#1D9BF0]'
            : '')
        }
      >
        <div className="flex flex-row space-x-2 my-auto w-full">
          {value.fileId ? (
            <>
              {urlLoading ? (
                <div className="h-12 w-[120px] flex flex-col justify-center items-center rounded bg-gray-200">
                  <svg
                ...
                <img
                  className="h-12 max-w-[120px] rounded"
                  src={googleUrl}
                />
              )}
            </>
...
function Drafts({
  ...
  useEffect(() => {
    if (refetchThreadDrafts) {
      getDraftThreads(workspaceId).then((value) => {
        setDraftsData(value);
        setRefetchThreadDrafts(false);
      });
    }
  }, [refetchThreadDrafts]);
      ...
      {draftsData?.length > 0 ? (
            ...
            .map((value: any, index: number) => {
              return (
                <Draft
                  ...
                  setRefetchDrafts={setRefetchThreadDrafts}
                  workspaceId={workspaceId}
                />
              );
            })}
        ...
        <div className="flex flex-col justify-center mt-4 items-center w-full">
          ...
          <p className="text-sm silka-medium mt-5 text-gray-900">
            No Draft Thread Found
          </p>
          <span className="text-xs silka-regular mt-1.5 text-gray-500">
            Being typing to create a draft thread
          </span>
        </div>
      )}
    </div>
...
function Media({ isLoading, mediaData, setTweetMedia }: MediaProps) {
  if (isLoading) {
    return (
      <>
        <div className="flex flex-row space-x-3">
    ...
    <div className="flex flex-col">
      <div className="flex flex-row justify-between items-between">
        ...
        <button
          ...
          className={
            'text-xs px-4 py-1 silka-regular text-gray-800 rounded hover:bg-gray-100 ' +
            (rightThreadSelected == 'Library' ? 'bg-gray-50' : '')
          }
        >
          Library
        </button>
      ...
      {rightThreadSelected == 'Drafts' ? (
        ...
        <Library
          workspaceId={workspaceId}
          tweetMedia={currentThreadTweetMedia}
          setTweetMedia={setCurrentThreadTweetMedia}
        />
      )}
    </div>
  );
