...
async function deleteDraft(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}twitter/delete/drafttweet`,
      null,
      {
        params: {
...
async function duplicateDraft(draftTweetId: string) {
  try {
    axiosRetry(axios, { retries: 3 });
    ...
    const result = await axios.post(
      `${apiUrl()}twitter/create/duplicatedrafttweet`,
      null,
      {
        params: { draftTweetId: draftTweetId },
      }
    );
    toast.remove();
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedTweet == value.id ? 'border border-[#1D9BF0]' : '')
        }
      >
          ...
          {value.fileId ? (
            <>
              {urlLoading ? (
                ...
                <img
                  className="h-12 max-w-[120px] my-auto rounded"
                  src={googleUrl}
                />
              )}
            </>
      ...
      {draftsData?.length > 0 ? (
        ...
        <div className="flex flex-col justify-center mt-4 items-center w-full">
          ...
          <p className="text-sm silka-medium mt-5 text-gray-900">
            No Drafts Tweet Found
          </p>
          <span className="text-xs silka-regular mt-1.5 text-gray-500">
            Begin typing to create a draft tweet
          </span>
        </div>
      )}
    </div>
    ...
    <div className="flex flex-col">
      <div className="flex flex-row justify-between items-between">
        ...
        <button
          ...
          className={
            'text-xs px-4 py-1 silka-regular text-gray-800 rounded hover:bg-gray-100 ' +
            (rightTweetSelected == 'Library' ? 'bg-gray-100' : '')
          }
        >
          Library
        </button>
