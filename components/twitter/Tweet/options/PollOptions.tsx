...
import { SchedulePollDialog } from '../../Dialog/SchedulePollDialog';
import { TweetPollNowDialog } from '../../Dialog/TweetPollNowDialog';
...
import { TwitterPollTimeSlots } from '../../Dialog/PollAvailableTimeSlots';
...
async function getOptions(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/draftpolloptions`,
      {
        params: {
          draftId: draftId,
        },
...
async function updateOptions(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}twitter/update/draftpolloptions`,
      null,
      {
        params: {
          draftId: draftId,
          removeUrlPreview: removeUrlPreview,
          pollMetric: pollMetric,
          pollInputValue: pollInputValue,
...
export function PollOptions({
  ...
  const [schedulePollOpen, setSchedulePollOpen] = useState(false);
  ...
  const [pollMetric, setPollMetric] = useState<string>('Hours');
  const [pollInputValue, setPollInputValue] = useState<number>(0);
  ...
  useEffect(() => {
    if (draftId) {
      getOptions(draftId).then((value) => {
        ...
        setPollMetric(value.duration_metric);
        setPollInputValue(value.duration);
  ...
  return (
    <div className="flex flex-col mt-6">
      <div className="flex flex-col justify-center items-center">
        <h3 className="text-xl silka-semibold text-gray-900 text-center">
          Poll Duration
        </h3>
        <p className="mt-2 text-xs silka-regular text-center text-gray-500">
          How long do you want the poll to run?
        </p>
        ...
        <input
          ...
          onChange={(e) => {
            if (e.target.value.length == 0) {
              ...
              let isnum =
                String(e.target.value).match(/^[0-9]+$/) != null;
              if (isnum) {
                setPollInputValue(parseInt(e.target.value));
              } else {
      ...
      <button
        ...
        className="flex flex-row space-x-1 mt-10 justify-start items-start"
      >
        <p className="text-sm silka-medium">Poll Options</p>
        {optionsOpen ? (
          <svg
            width="15"
            height="15"
          ...
          <div className="flex flex-col space-y-4">
            ...
            {autoRetweetOpen ? (
              <div className="flex flex-col justify-center items-center relative">
                ...
                >
                  <h3 className="text-xl silka-semibold text-center text-gray-900">
                    Auto Retweet
                  </h3>
                  <p className="mt-2 text-xs silka-regular text-gray-500 text-center">
      ...
      <div className="flex flex-row justify-center items-center mt-6 space-x-5">
        <div>
          <DialogPrimitive.Root
            open={tweetNowOpen}
        ...
        <div>
          <DialogPrimitive.Root
            open={schedulePollOpen}
            onOpenChange={setSchedulePollOpen}
          >
            <DialogPrimitive.Trigger asChild>
              <button
                onClick={(e) => {
            ...
            <SchedulePollDialog
              isOpen={schedulePollOpen}
              setIsOpen={setSchedulePollOpen}
              tweetText={tweetText}
              pollFields={pollFields}
              workspaceId={workspaceId}
              draftId={draftId}
      ...
      >
        <TwitterPollTimeSlots
          isOpen={timeSlotsOpen}
          setIsOpen={setTimeSlotsOpen}
          setScheduleOpen={setSchedulePollOpen}
          workspaceId={workspaceId}
          draftId={draftId}
          userId={String(session?.user?.id)}
          setSelectedPoll={setSelectedPoll}
          setRefetchPollDrafts={setRefetchDrafts}
        />
      </DialogPrimitive.Root>
    </div>
  );
