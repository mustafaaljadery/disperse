...
async function getSearch(workspaceId: string, searchQuery: string) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(
      `${apiUrl()}twitter/read/searchtweets`,
      {
        params: {
...
function TweetAndReplyComponent({
  workspaceId,
  value,
  index,
  user,
  ...
  return (
    <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row md:space-x-5 w-full">
        ...
        <div className="flex flex-col">
          <div className="flex flex-row space-x-3">
            <img
              ...
              className="rounded-full h-[42px] w-[42px] md:w-[28px] md:h-[28px] xl:h-[42px] xl:w-[42px]"
            />
          ...
          <div className="mt-4 flex flex-row justify-between items-between">
            <div className="flex flex-row space-x-2 md:space-x-0.5 xl:space-x-4 my-auto">
              <button
                onMouseEnter={() => {
                  setLikeHovered(true);
                }}
                ...
                className={
                  'flex flex-row space-x-2 px-3 md:px-1.5 xl:px-4 py-1 lg:py-1.5 rounded ' +
            ...
            <div className="flex flex-row my-auto space-x-4 md:space-x-2 xl:space-x-4">
              <button
                onClick={(e) => {
                  e.preventDefault();
                  addRemoveTweet(workspaceId, value.id);
      ...
      <form
        ...
        className="w-1/2 my-auto flex flex-col space-y-2"
      >
        <textarea
          value={text}
          onChange={(e) => {
...
export function SearchKeywords({ workspaceId }: Props) {
    ...
    <div className="flex flex-col justify-center items-center w-full">
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault();
          setIsLoading(true);
          getSearch(workspaceId, search).then((value) => {
        ...
        className="flex flex-row bg-gray-100 mt-6 rounded"
      >
        <input
          required
          type="text"
          ...
          className="w-[440px] bg-gray-100 text-sm py-2.5 px-3 rounded silka-medium text-gray-900 border-none focus:ring-0"
        />
        <button
          type="submit"
          className="p-3 rounded-r hover:bg-gray-200 rounded"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
      ...
      <div className="flex flex-col mt-12 space-y-10 w-full mb-24">
        {isLoading ? (
          <>
          ...
          <div className="h-72 rounded-xl w-full border border-gray-300 border-dashed flex flex-col justify-center items-center">
            ...
            <p className="text-sm silka-regular w-1/2 text-center mt-3 text-gray-400">
              Search and reply to tweets, to build up your social
              following and engage with more users.
            </p>
          </div>
        ) : tweets.length == 0 ? (
          <div className="h-72 rounded-xl w-full border border-gray-300 border-dashed flex flex-col justify-center items-center">
            ...
            <p className="text-sm silka-regular w-1/2 text-center mt-3 text-gray-400">
              You are up to date with all of your followers, search
              new tweets.
            </p>
          </div>
        ) : (
