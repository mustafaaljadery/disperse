...
async function createScheduledTweet(
  workspaceId: string,
  userId: string,
  draftId: string,
  date: string,
  ...
  return (
    <div className="h-10 px-3 w-full flex flex-row justify-between items-between border rounded-lg">
      ...
      <button
        ...
        className="text-[#1D9BF0] silka-medium text-sm hover:opacity-80"
      >
        Select Time
      </button>
    </div>
...
async function getAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}twitter/read/availableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}twitter/read/moreavailableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
export function AvailableTimeSlotsDialog({
  isOpen,
  setIsOpen,
  setScheduleOpen,
  workspaceId,
          ...
          <div className="flex flex-col w-full">
            ...
            {isLoading ? (
              ...
              <div className="h-[400px] mt-6 w-full">
                <Scroll>
                  <div className="flex flex-col space-y-6 px-3 py-2">
                    {scheduleData?.map(
