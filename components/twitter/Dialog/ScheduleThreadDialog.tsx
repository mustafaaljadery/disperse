...
async function nextTimeSlot(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}twitter/create/threadnexttimeslot`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Successfully Scheduled Thread!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
...
interface ScheduleThreadDialogProps {
  isOpen: boolean;
...
export function ScheduleThreadDialog({
  isOpen,
  workspaceId,
  draftId,
    ...
    <Transition.Root show={isOpen}>
      ...
      >
        ...
        >
          ...
          <div className="mt-7 flex flex-col space-y-2.5 w-full">
            <button
              onClick={(e) => {
                e.preventDefault();
                setIsOpen(false);
