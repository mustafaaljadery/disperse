...
interface TwitterFollowersChartProps {
  timeInterval: string;
  followers: any;
  workspaceId: string;
  isPremium: boolean;
...
async function getDailyFollowers(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/dailyfollowers`,
      {
        params: {
...
async function getTotalFollowers(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/totalfollowers`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function TwitterFollowersChart({
  timeInterval,
  followers,
  workspaceId,
  isPremium,
}: TwitterFollowersChartProps) {
      ...
      if (isPremium) {
        ...
        if (selectedFormat == 'Daily') {
          ...
          getTotalFollowers(
            ...
            setTotalFollowersData(temp2);
            setIsLoading(false);
          });
        }
      } else {
        if (selectedFormat == 'Daily') {
          const value = randomData(timeInterval, followers);
          setDailyFollowersData(value);
          setIsLoading(false);
        } else {
          const value = randomData(timeInterval, followers);
          setTotalFollowersData(value);
          setIsLoading(false);
        }
      }
    }
