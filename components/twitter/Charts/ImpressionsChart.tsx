...
interface TwitterImpressionsChartProps {
  timeInterval: string;
  workspaceId: string;
  impressions: any;
  isPremium: boolean;
  ...
  datasets: [
    {
      label: 'Impressions',
      data: [
        0.2, 0.2, 3, 5, 2, 3, 12, 19, 3, 5, 2, 3, 12, 19, 3, 5, 2, 3,
        12, 19, 3, 5, 2, 3, 12, 19, 3, 5, 2, 3, 12, 19, 3, 5, 2, 3,
        12, 19, 3, 5, 2, 3, 12, 19, 3, 5, 2, 3, 12, 19, 3, 5, 2, 3,
      ],
...
async function getImpressions(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/dailyimpressions`,
      {
        params: {
...
export function TwitterImpressionsChart({
  ...
  const [impressionsData, setImpressionsData] = useState<any>(null);
  ...
  useEffect(() => {
    if (impressions) {
      if (isPremium) {
        setIsLoading(true);
        getImpressions(workspaceId, timeInterval, impressions).then(
          (value) => {
            let temp = [];
        ...
        const value = randomData(timeInterval, impressions);
        setImpressionsData(value);
        setIsLoading(false);
      }
    }
  }, [impressions, timeInterval]);
