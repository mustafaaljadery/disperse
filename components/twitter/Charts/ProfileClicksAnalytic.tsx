...
interface ProfileClicksAnalyticProps {
  workspaceId: string;
  profileClicks: any;
  isPremium: boolean;
}
...
async function getProfileClicks(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/profileclicks`,
      {
        params: {
...
export function ProfileClicksAnalytic({
  workspaceId,
  profileClicks,
  isPremium,
}: ProfileClicksAnalyticProps) {
  const [isLoading, setIsLoading] = useState(true);
  const [profileClicksCount, setProfileClicksCount] = useState(0);
  const [chartData, setChartData] = useState<any>(null);
  const [percentChange, setPrecentChange] = useState(0);

  useEffect(() => {
    if (profileClicks != null) {
      if (isPremium) {
        setProfileClicksCount(profileClicks);
        getProfileClicks(workspaceId, profileClicks).then((value) => {
          setPrecentChange(value.percentChange.toFixed(2));
          let temp: any = [];
          for (let i = 0; i < value.data.length; i++) {
            temp.push(value.data[i].clicks);
          }
          setChartData(temp);
          setIsLoading(false);
        });
        ...
        let change: any = parseInt(profileClicks) / second_to_last;
        setProfileClicksCount(profileClicks);
        if (profileClicks < second_to_last)
          change = -Math.abs(1 - change);
        setPrecentChange(change.toFixed(1));
        setChartData([
          between(1, 20000),
          between(1, 20000),
          between(1, 20000),
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(profileClicksCount, 2)}
          </p>
        )}
        {isLoading ? (
