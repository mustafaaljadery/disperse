...
async function getEngagementData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/engagementschart`,
      {
        params: {
...
async function getEngagementRate(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/engagementratechart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function TwitterEngagementChart({
  ...
  const [engagementNumber, setEngagementNumber] = useState(0);
  ...
  useEffect(() => {
    if (engagements != null && impressions != null) {
      setEngagementNumber(engagements);
      setEngagementRate((engagements / impressions) * 100);
      setIsLoading(true);
      if (isPremium) {
        ...
        if (selectedFormat == 'Total') {
          setEngagementsData(
            randomData(timeInterval, engagements, 'number')
          );
          setIsLoading(false);
        } else {
  ...
  }, [selectedFormat, timeInterval, engagements, impressions]);
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {selectedFormat == 'Total'
                  ? formatLargeNumber(engagementNumber, 2)
