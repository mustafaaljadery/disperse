...
async function getTweets(workspaceId: string, tweets: number) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}twitter/read/tweets`, {
      params: { workspaceId: workspaceId, tweets: tweets },
    });
    return result.data;
  } catch (e) {
    console.log(e);
...
export function TweetsAnalytic({
  workspaceId,
  tweets,
  isPremium,
}: TweetsAnalyticProps) {
  ...
  useEffect(() => {
    if (tweets != null) {
      if (isPremium) {
        ...
        getTweets(workspaceId, tweets).then((value) => {
        ...
        let change: any = parseInt(tweets) / second_to_last;
        setTweetsCount(tweets);
        if (tweets < second_to_last) change = -Math.abs(1 - change);
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(tweetsCount, 1)}
          </p>
        )}
        {isLoading ? (
