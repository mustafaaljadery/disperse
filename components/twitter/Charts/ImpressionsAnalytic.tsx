...
interface ImpressionsAnalyticProps {
  workspaceId: string;
  impressions: any;
  isPremium: boolean;
}
...
async function getImpressions(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/impressions`,
      {
        params: {
...
export function ImpressionsAnalytic({
  workspaceId,
  impressions,
  isPremium,
}: ImpressionsAnalyticProps) {
  ...
  useEffect(() => {
    if (impressions != null) {
      if (isPremium) {
        ...
        getImpressions(workspaceId, impressions).then((value) => {
          setPercentChange(value.percentChange.toFixed(2));
          let temp: any = [];
          for (let i = 0; i < value.data.length; i++) {
            temp.push(value.data[i].impressions);
          }
          setChartData(temp);
          setIsLoading(false);
        });
        ...
        let change: any = parseInt(impressions) / second_to_last;
        setImpressionsCount(impressions);
        if (impressions < second_to_last)
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(impressionsCount, 1)}
          </p>
        )}
        {isLoading ? (
