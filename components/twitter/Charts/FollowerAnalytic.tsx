...
interface FollowersAnalyticProps {
  workspaceId: string;
  followers: any;
  isPremium: boolean;
}
...
async function getFollowersAnalytic(
  workspaceId: string,
  followers: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}twitter/read/followers`,
      {
        params: { workspaceId: workspaceId, followers: followers },
      }
    );
    return result.data;
  } catch (e) {
...
export function FollowerAnalytic({
  workspaceId,
  followers,
  isPremium,
}: FollowersAnalyticProps) {
  ...
  useEffect(() => {
    if (followers != null) {
      if (isPremium) {
        ...
        getFollowersAnalytic(workspaceId, followers).then((value) => {
          setPercentChange(value.percentChange.toFixed(2));
          let temp: any = [];
          for (let i = 0; i < value.data.length; i++) {
            temp.push(value.data[i].followers);
          }
          setChartData(temp);
          setIsLoading(false);
        });
        ...
        let change: any = parseInt(followers) / second_to_last;
        setFollowersCount(followers);
        if (followers < second_to_last)
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(followersCount, 1)}
          </p>
        )}
        {isLoading ? (
