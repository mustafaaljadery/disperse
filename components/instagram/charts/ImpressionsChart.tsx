...
async function getImpressionsData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/impressionschart`,
      {
        params: {
...
async function getProfileViewsData(
  workspaceId: string,
  timeInterval: string,
  profileViews: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/profileviewschart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
async function getReachData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/reachchart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function InstagramImpressionsChart({
  workspaceId,
  timeInterval,
  impressions,
  profileViews,
  ...
  const [profileViewsData, setProfileViewsData] = useState<any>([]);
  const [reachData, setReachData] = useState<any>([]);
  const [selectedFormat, setSelectedFormat] = useState('Impressions');
      ...
      if (isPremium) {
        ...
        if (selectedFormat == 'Impressions') {
          getImpressionsData(
            ...
            setImpressionsData(temp);
            setIsLoading(false);
          });
        } else if (selectedFormat == 'Profile Views') {
          getProfileViewsData(
            workspaceId,
            timeInterval,
            profileViews
            ...
            setProfileViewsData(temp);
            setIsLoading(false);
          });
        } else {
          getReachData(workspaceId, timeInterval, reach).then(
            (value) => {
              let temp = [];
              for (let i = 0; i < value.length; i++) {
                if (value[i] < 0) {
        ...
        setIsLoading(true);
        if (selectedFormat == 'Impressions') {
          setImpressionsData(randomData(timeInterval, impressions));
          setIsLoading(false);
        } else if (selectedFormat == 'Profile Views') {
          setProfileViewsData(randomData(timeInterval, profileViews));
          setIsLoading(false);
        } else {
          setReachData(randomData(timeInterval, reach));
          setIsLoading(false);
        }
      }
    }
  ...
  const data = {
    labels:
      selectedFormat == 'Impressions'
        ? impressionsData
        : selectedFormat == 'Profile Views'
        ? profileViewsData
        : reachData,
    datasets: [
      {
      ...
      <div className="flex flex-row justify-between items-between">
        ...
        <div className="flex flex-row space-x-3">
          ...
          <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() => {
                setSelectedFormat('Profile Views');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Profile Views'
            ...
            <button
              onClick={() => {
                setSelectedFormat('Reach');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Reach'
                  ? 'bg-white shadow text-gray-800'
                  : 'text-gray-500')
              }
            >
