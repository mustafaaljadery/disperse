...
async function getProfileViewsAnalytic(
  workspaceId: string,
  profileViews: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/profileviews`,
      {
        params: {
...
export function InstagramProfileViewsAnalytic({
  workspaceId,
  profileViews,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (profileViews != null) {
      if (isPremium) {
        setProfileViewsCount(profileViews);
        getProfileViewsAnalytic(workspaceId, profileViews).then(
          (value) => {
        ...
        let change: any = parseInt(profileViews) / second_to_last;
        setProfileViewsCount(profileViews);
        if (profileViews < second_to_last)
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(profileViewsCount, 1)}
          </p>
        )}
        {isLoading ? (
