...
async function getCommentsData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/commentschart`,
      {
        params: {
...
async function getSharesData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/shareschart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function InstagramCommentsChart({
  workspaceId,
  timeInterval,
  comments,
  shares,
  ...
  const [selectedFormat, setSelectedFormat] = useState('Shares');
  ...
  useEffect(() => {
    if (shares != null && comments != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Shares') {
          getSharesData(workspaceId, timeInterval, shares).then(
            (value) => {
              let temp = [];
          ...
          getCommentsData(workspaceId, timeInterval, comments).then(
            (value) => {
              let temp = [];
              for (let i = 0; i < value.length; i++) {
                if (value[i] < 0) {
        ...
        if (selectedFormat == 'Shares') {
          const value = randomData(timeInterval, shares);
          setSharesData(value);
          setIsLoading(false);
        } else {
          const value = randomData(timeInterval, comments);
  ...
  const data = {
    labels: selectedFormat == 'Shares' ? sharesData : commentsData,
    datasets: [
      {
        label: selectedFormat == 'Shares' ? 'Shares' : 'Comments',
        data: selectedFormat == 'Shares' ? sharesData : commentsData,
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(
                  selectedFormat == 'Shares' ? shares : comments,
                  1
                )}
              </p>
            )}
        ...
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              chartData={
                selectedFormat == 'Shares' ? sharesData : commentsData
              }
