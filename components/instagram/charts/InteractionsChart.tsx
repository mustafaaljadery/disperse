...
async function getTotalInteractions(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/totalinteractions`,
      {
        params: {
...
async function getDailyInteractions(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/dailyinteractions`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function InstagramInteractionsChart({
  workspaceId,
  timeInterval,
  interactions,
  isPremium,
  ...
  const [dailyInteractionsData, setDailyInteractionsData] =
    useState<any>(null);
  const [totalInteractionsData, setTotalInteractionsData] =
    useState<any>(null);
  ...
  useEffect(() => {
    if (interactions != null) {
      if (isPremium) {
        ...
        setIsLoading(true);
        if (selectedFormat == 'Daily') {
          setDailyInteractionsData(
            randomData(timeInterval, interactions)
          );
          setIsLoading(false);
        } else {
  ...
  }, [selectedFormat, timeInterval, interactions]);
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(interactions, 1)}
              </p>
            )}
          </div>
        </div>
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              chartName={'Instagram Interactions'}
              chartValue={interactions}
              chartData={
                selectedFormat == 'Total'
                  ? totalInteractionsData
                  : dailyInteractionsData
              }
              labelName={'Interactions'}
              timeInterval={timeInterval}
