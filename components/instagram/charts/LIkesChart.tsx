...
async function getTotalLikes(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/totallikes`,
      {
        params: {
...
async function getDailyLikes(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/dailylikes`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function InstagramLikesChart({
  workspaceId,
  timeInterval,
  likes,
  isPremium,
  ...
  const [dailyLikesData, setDailyLikesData] = useState<any>(null);
  const [totalLikesData, setTotalLikesData] = useState<any>(null);
    ...
    if (likes != null) {
      if (isPremium) {
        ...
        if (selectedFormat == 'Daily') {
          getDailyLikes(workspaceId, timeInterval, likes).then(
            (value) => {
              let temp = [];
              ...
              setDailyLikesData(temp);
              setIsLoading(false);
            }
          );
        } else {
          getTotalLikes(workspaceId, timeInterval, likes).then(
            (value) => {
              let temp = [];
              for (let i = 0; i < value.length; i++) {
                if (value[i] < 0) {
        ...
        setIsLoading(true);
        if (selectedFormat == 'Daily') {
          setDailyLikesData(randomData(timeInterval, likes));
          setIsLoading(false);
        } else {
          setTotalLikesData(randomData(timeInterval, likes));
          setIsLoading(false);
        }
      }
    }
  }, [selectedFormat, timeInterval, likes]);

  const data = {
    labels:
      selectedFormat == 'Daily' ? dailyLikesData : totalLikesData,
    datasets: [
      {
        label: 'Likes',
        data:
