...
async function getEngagements(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/engagementschart`,
      {
        params: {
...
async function getEngagementRate(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/engagementratechart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function InstagramEngagementChart({
  ...
  useEffect(() => {
    if (engagement != null && impressions != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Engagements') {
          getEngagements(workspaceId, timeInterval, engagement).then(
            (value) => {
              let temp = [];
        ...
        if (selectedFormat == 'Engagements') {
          const value = randomData(timeInterval, engagement, 'total');
          setEngagementData(value);
          setIsLoading(false);
        } else {
          const value = randomData(
            timeInterval,
  ...
  }, [selectedFormat, timeInterval, impressions, engagement]);
