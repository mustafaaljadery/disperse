...
async function getImpressionsAnalytic(
  workspaceId: string,
  impressions: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/impressions`,
      {
        params: {
...
export function InstagramImpressionsAnalytic({
  workspaceId,
  impressions,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (impressions != null) {
      if (isPremium) {
        ...
        getImpressionsAnalytic(workspaceId, impressions).then(
          (value) => {
