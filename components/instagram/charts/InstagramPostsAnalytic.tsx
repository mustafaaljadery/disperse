...
async function getPostsAnalytic(workspaceId: string, posts: number) {
  // Analytics
  axiosRetry(axios, { retries: 3 });
  try {
    const result = await axios.get(
      `${apiUrl()}instagram/read/posts`,
      {
        params: {
...
export function InstagramPostsAnalytic({
  workspaceId,
  posts,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (posts != null) {
      if (isPremium) {
        ...
        let change: any = parseInt(posts) / second_to_last;
        setPostsCount(posts);
