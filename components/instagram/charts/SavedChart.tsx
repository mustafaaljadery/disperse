...
async function getWebsiteClicksData(
  workspaceId: string,
  timeInterval: string,
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/websiteclickschart`,
      {
        params: {
...
async function getSavedData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/savedchart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function InstagramSavedChart({
  workspaceId,
  timeInterval,
  saved,
  websiteClicks,
  ...
  const [websiteClicksData, setWebsiteClicksData] =
    useState<any>(null);
  const [savedData, setSavedData] = useState<any>(null);
  const [selectedFormat, setSelectedFormat] = useState('Saved');
  const [screenshot, setScreenshot] = useState(false);

  useEffect(() => {
    if (saved != null && websiteClicks != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Saved') {
          getSavedData(workspaceId, timeInterval, saved).then(
            (value) => {
              let temp = [];
          ...
          getWebsiteClicksData(
            ...
            setWebsiteClicksData(temp);
            setIsLoading(false);
          });
        }
      } else {
        if (selectedFormat == 'Saved') {
          setSavedData(randomData(timeInterval, saved));
          setIsLoading(false);
        } else {
          setWebsiteClicksData(
            randomData(timeInterval, websiteClicks)
          );
          setIsLoading(false);
        }
      }
    }
  }, [selectedFormat, timeInterval, saved, websiteClicks]);

  const data = {
    labels: selectedFormat == 'Saved' ? savedData : websiteClicksData,
    datasets: [
      {
        label: selectedFormat == 'Saved' ? 'Saved' : 'Website Clicks',
        data:
          selectedFormat == 'Saved' ? savedData : websiteClicksData,
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(
                  selectedFormat == 'Saved' ? saved : websiteClicks,
                  1
                )}
              </p>
            )}
        ...
        <div className="flex flex-row space-x-3">
          ...
          >
            ...
            <DownloadChart
              ...
              labelName={
                selectedFormat == 'Saved' ? 'Saved' : 'Clicks'
              }
              timeInterval={timeInterval}
              chartType={'bar'}
            />
          ...
          <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() => {
                setSelectedFormat('Website Clicks');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Website Clicks'
                  ? 'bg-white shadow text-gray-800'
                  : 'text-gray-500')
              }
            >
