...
async function createScheduledCarousel(
  workspaceId: string,
  userId: string,
  draftId: string,
  date: string,
  ...
  try {
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Successfully Scheduled Carousel!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    } else {
      toast.remove();
  ...
  return (
    <div className="h-10 px-3 w-full flex flex-row justify-between items-between border rounded-lg">
      ...
      <button
        ...
        className="text-[#F604D0] silka-medium text-sm hover:opacity-80"
      >
        Select Time
      </button>
    </div>
...
async function getAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}instagram/read/availableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}instagram/read/moreavailableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
export function InstagramCarouselTimeSlots({
  workspaceId,
  userId,
  draftId,
  isOpen,
    ...
    <Transition.Root show={isOpen}>
      ...
      >
        <DialogPrimitive.Content
          ...
          className={cx(
            'fixed z-50',
            'w-[95vw] max-w-lg rounded-lg py-4 px-2 md:w-full',
          ...
          <div className="flex flex-col w-full">
            ...
            {isLoading ? (
              ...
              <div className="h-[400px] mt-6 w-full">
                <Scroll>
                  <div className="flex flex-col space-y-6 py-2 px-2">
                    {scheduleData?.map(
