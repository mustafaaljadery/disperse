...
async function postReelNow(
  ...
  try {
    ...
    toast.loading('Posting Reel...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });
    const result = await axios.post(
      `${apiUrl()}instagram/create/postreel`,
      null,
      { params: { workspaceId: workspaceId, draftId: draftId } }
    );
    if (result.data.message == 'success') {
      ...
      toast.remove();
      toast.error('Error posting reel, please try again.', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
...
export function InstagramPostReelNow({
  isOpen,
  setIsOpen,
  autoRepostOpen,
  repostMetric,
          ...
          >
            <div className="w-full flex flex-col justify-center items-center">
              ...
              <p className="mt-2.5 text-xs silka-regular text-gray-400">
                Post reel on Instagram right now.
              </p>
            </div>
                ...
                <button
                  onClick={(e) => {
                    ...
                    if (autoRepostOpen && repostInputValue == 0) {
                      ...
                      toast.error(
                        'Please select a reel to be posted.',
                        {
                          className:
                            'silka-medium text-sm text-gray-900',
                        }
