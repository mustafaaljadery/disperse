...
import { InstagramVideoOptions } from '../options/VideoOptions';
...
async function createDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/create/draftvideo`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/videodraftdetails`,
      {
...
async function updateDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/update/draftvideo`,
      null,
      {
        params: {
          draftId: draftId,
...
export function InstagramVideoLeft({
  workspaceId,
  selectedVideo,
  setSelectedVideo,
  videoMedia,
      ...
      {videoMedia ? (
        ...
        >
          ...
          <p className="text-[10px] silka-regular text-gray-400">
            Pick a video to post on Instagram
          </p>
        </button>
      )}
      <textarea
