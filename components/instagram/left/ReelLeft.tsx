...
import { InstagramReelOptions } from '../options/ReelOptions';
...
async function createDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/create/draftreel`,
      null,
      {
        params: {
...
async function getDraftDetails(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/reeldraftdetails`,
      {
...
async function updateDraftVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/update/draftreel`,
      null,
      {
        params: {
          draftId: draftId,
...
export function InstagramReelLeft({
  workspaceId,
  selectedReel,
  setSelectedReel,
  reelMedia,
      ...
      {reelMedia ? (
        ...
        <div className="flex flex-row justify-center items-center w-full">
          <button
            ...
            className="mt-9 w-[260px] h-[462px] flex flex-col space-y-2 justify-center items-center border rounded-xl border-dashed  hover:border-[#F604D0]"
          >
