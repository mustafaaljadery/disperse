...
async function getProjectContent(folderId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/reelmedia`,
...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/create/duplicatedraftreel`,
      null,
      {
        params: {
          draftId: draftId,
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/delete/draftreel`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedReel == value.id ? 'border border-[#F604D0]' : '')
        }
      >
        {value.video == 'yes' ? (
    ...
    <div>
      {mediaData?.length > 0 ? (
        ...
        <div className="flex flex-col justify-center items-center border border-dashed h-36 rounded-lg">
          ...
          <span className="mt-1.5 silka-regular text-[11px] text-gray-400">
...
export function InstagramReelRight({
  workspaceId,
  selectedReel,
  setSelectedReel,
  reelMedia,
