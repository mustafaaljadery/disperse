...
async function getProjectContent(folderId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/videomedia`,
...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/create/duplicatedraftvideo`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    toast.remove();
    toast.success('Successsfully Duplicated Draft!', {
      className: 'text-sm silka-medium text-gray-900',
    });
    return result.data;
  } catch (e) {
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/delete/draftvideo`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    toast.remove();
    toast.success('Successsfully Deleted Draft!', {
      className: 'text-sm silka-medium text-gray-900',
    });
    return result.data;
  } catch (e) {
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedVideo == value.id ? 'border border-[#F604D0]' : '')
        }
      >
        {value.video == 'yes' ? (
...
function Media({ isLoading, mediaData, setVideoMedia }: MediaProps) {
  if (isLoading) {
    return (
      <>
        <div className="flex flex-row space-x-3">
...
function Library({
  ...
  return (
    <>
      <div className="mt-4 py-3 flex flex-row space-x-2.5 rounded-lg px-4 bg-[#F6E7F3]">
        <svg
          width="18"
          height="18"
          viewBox="0 0 24 24"
        ...
        <p className="silka-semibold my-auto text-[#F604D0] text-xs">
          Make sure Instagram video dimensions are as close to 1:1 as
          possible.
        </p>
      </div>
...
export function InstagramVideoRight({
  workspaceId,
  selectedVideo,
  setSelectedVideo,
  videoMedia,
