...
async function duplicateDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/create/duplicatedraftimage`,
      null,
      {
        params: {
          draftId: draftId,
...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}instagram/delete/draftimage`,
      null,
      {
        params: {
          draftId: draftId,
    ...
    <div className="w-full xl:w-1/2 p-2">
      <button
        ...
        className={
          ...
          (selectedImage == value.id ? 'border border-[#F604D0]' : '')
        }
      >
        {value.image == 'yes' ? (
    ...
    <div className="mt-6 w-full flex flex-row space-x-7 flex-wrap">
      {projectsData.length == 0 ? (
        <div className="flex flex-col justify-center mt-4 items-center w-full">
          ...
          <h3 className="text-sm mt-3 silka-semibold text-gray-900">
            No Media Found
          </h3>
...
export function InstagramImageRight({
  workspaceId,
  selectedImage,
  setSelectedImage,
  imageMedia,
