...
async function deleteDraft(draftId: string, workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}youtube/delete/shortdraft`,
      null,
      {
          ...
          <>
            {thumbnailLoading ? (
              <div className="flex flex-col justify-center items-center w-full">
                <div className="bg-gray-200 h-32 w-24 rounded flex flex-col justify-center items-center">
                  <svg
...
export function YoutubeShortsDrafts({
  workspaceId,
  setDraftId,
  refetchShortsDrafts,
  setRefetchShortsDrafts,
  ...
  useEffect(() => {
    if (refetchShortsDrafts) {
      getDrafts(workspaceId).then((value) => {
        setDraftsData(value);
        setRefetchShortsDrafts(false);
      });
    }
  }, [refetchShortsDrafts]);
  ...
  if (isLoading) {
    return (
      <div className="h-full flex flex-col mt-7 space-y-1.5">
