...
import { VideoCategoryDropdown } from './dropdown/VideoCategoryDropdown';
import { VideoLangugageDropdown } from './dropdown/VideoLanguageDropdown';
...
async function createDraft(workspaceId: string) {
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}youtube/create/videodraft`,
      null,
      {
...
async function getDraft(draftId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/videodraft`,
      {
...
async function updateDraft(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}youtube/update/videosdraft`,
      null,
      {
        params: {
          draftId: draftId,
...
export function YoutubeVideoLeft({
  workspaceId,
  section,
  setSection,
  draftId,
      ...
      <div className="flex flex-col mt-8">
        ...
        {videoMedia == null ? (
          <button
            ...
            className="mt-4 w-full flex flex-col justify-center items-center border rounded-lg border-dashed h-64 hover:border-[#FF623D]"
          >
            <p className="text-xs silka-semibold text-gray-900 mt-3">
              Youtube Video
            </p>
            <p className="text-[10px] silka-regular mt-1 text-gray-400">
              Select media to upload video
            </p>
          </button>
        ) : (
          <>
            <video
              className="h-64 mt-2 w-full rounded-lg bg-black"
              controls
              src={url}
            />
          </>
