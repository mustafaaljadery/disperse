...
import { YoutubeScheduleTimesDialog } from './YoutubeScheduleTimesDialog';
import { YoutubePickVideo } from './dialog/PickVideo';
...
async function unscheduleVideo(
  ...
  try {
    ...
    const result = await axios.post(
      `${apiUrl()}youtube/delete/scheduledvideo`,
      null,
      {
        params: {
...
async function unscheduleShort(
  scheduledId: string,
  workspaceId: string,
  userId: string
) {
  try {
    axiosRetry(axios, { retries: 3 });
    toast.loading('Unscheduling Short...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });

    const result = await axios.post(
      `${apiUrl()}youtube/delete/scheduledshort`,
      null,
      {
        params: {
          scheduledId: scheduledId,
    ...
    if (result.data.message == 'success') {
      toast.remove();
      toast.success('Short successfully unscheduled!', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    ...
    toast.remove();
    toast.error('Error unscheduling short, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
    return 'error';
...
async function getScheduledVideos(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/scheduledvideos`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreTweets(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/morescheduled`,
      {
        params: {
          workspaceId: workspaceId,
          cursor: cursor,
          ...
          {hovered ? (
            <div className="my-auto">
              <DialogPrimitive.Root
                ...
                onOpenChange={setSelectVideoOpen}
              >
                <DialogPrimitive.Trigger asChild>
                  <button className="my-auto py-1 px-3 rounded text-xs hover:bg-[#F6E7E7] hover:opacity-80 silka-semibold text-[#FF0000]">
                    Pick Video
                  </button>
  ...
  return (
    <div className="flex flex-col space-y-3">
      ...
      {value.schedule_slots.length == 0 ? (
        <div className="h-11 px-3 w-full flex flex-row justify-between border rounded-lg">
...
export function YoutubeScheduledVideos({ workspaceId }: Props) {
  const [isLoading, setIsLoading] = useState(true);
  const [changeScheduleOpen, setChangeScheduleOpen] = useState(false);
  const [scheduleData, setScheduleData] = useState<any>(null);
  const [refetchScheduledVideos, setRefetchScheduledVideos] =
    useState(false);
  ...
  useEffect(() => {
    if (isVisible && !moreLoading) {
      ...
      getMoreTweets(
        ...
        setCursor(cursor + 1);
        setScheduleData(scheduleData.concat(value.scheduled_videos));
  ...
  useEffect(() => {
    if (status == 'authenticated') {
      getScheduledVideos(workspaceId, session.user.id).then(
        (value) => {
          setScheduleData(value.scheduled_videos);
  ...
  useEffect(() => {
    if (refetchScheduledVideos && status == 'authenticated') {
      getScheduledVideos(workspaceId, session?.user?.id).then(
        (value) => {
          setScheduleData(value.scheduled_videos);
          setTimezone(value.timezone);
        ...
        <div className="flex flex-col space-y-5 md:space-y-0 md:flex-row md:justify-between md:items-between">
          <div className="flex flex-col">
            ...
            <p className="text-xs silka-regular text-gray-500">
              Upcoming videos scheduled to be posted on Youtube.
            </p>
          </div>
          <div className="flex flex-row space-x-4">
              ...
              <div className="my-auto">
                ...
                >
                  ...
                  <MyTimezoneDialog
                    ...
                    setRefetchScheduled={setRefetchScheduledVideos}
                  />
                </DialogPrimitive.Root>
              </div>
            )}
            <div className="my-auto">
              ...
              >
                ...
                <YoutubeScheduleTimesDialog
                  isOpen={changeScheduleOpen}
                  workspaceId={workspaceId}
                  userId={String(session?.user?.id)}
                  refetchScheduledVideos={refetchScheduledVideos}
            ...
            {scheduleData?.map((value: any, index: number) => {
              return (
                <ScheduleDate
                  ...
                  value={value}
                  key={index}
                  setRefetchVideos={setRefetchScheduledVideos}
                  userId={String(session?.user?.id)}
                />
              );
            })}
