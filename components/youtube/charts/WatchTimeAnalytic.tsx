...
async function getWatchTimeData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/watchtime`,
      {
        params: { workspaceId: workspaceId, watchtime: watchtime },
      }
    );
    return result.data;
  } catch (e) {
...
export function YoutubeWatchTimeAnalytic({
  workspaceId,
  watchtime,
  isPremium,
}: Props) {
  const [isLoading, setIsLoading] = useState(true);
  const [watchTimeCount, setWatchTimeCount] = useState(0);
  ...
  useEffect(() => {
    if (watchtime != null) {
      if (isPremium) {
        setWatchTimeCount(watchtime);
        getWatchTimeData(workspaceId, watchtime).then((value) => {
        ...
        let change: any = parseInt(watchtime) / second_to_last;
        setWatchTimeCount(watchtime);
        if (watchtime < second_to_last)
  ...
  return (
    <div className="w-1/2 sm:w-1/3 py-3 md:py-0 md:w-1/5 flex flex-col justify-start items-start space-y-1.5 px-6 lg:px-4 xl:px-6">
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          <div className="h-9 lg:h-7 2xl:h-9 w-20 lg:w-14 2xl:w-20 animate-pulse rounded bg-gray-200" />
        ) : (
          <p className="silka-semibold text-4xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(watchTimeCount, 1)}
          </p>
        )}
        {isLoading ? (
