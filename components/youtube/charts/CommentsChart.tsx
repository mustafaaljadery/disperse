...
async function getCommentsData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/commentschart`,
      {
        params: {
...
async function getSharesData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/shareschart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function YoutubeCommentsChart({
  workspaceId,
  timeInterval,
  comments,
  shares,
      ...
      if (isPremium) {
        if (selectedFormat == 'Comments') {
          getCommentsData(workspaceId, timeInterval, comments).then(
            (value) => {
              // if a single value is less than zero, then make it zero
              let temp = [];
        ...
        if (selectedFormat == 'Comments') {
          let value = randomData(timeInterval, comments);
          setCommentsData(value);
          setIsLoading(false);
        } else {
          let value = randomData(timeInterval, shares);
          setSharesData(value);
          setIsLoading(false);
        }
      }
