...
async function getSubscribersData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/subscribers`,
      {
        params: {
...
export function YoutubeSubscribersAnalytic({
  workspaceId,
  subscribers,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (subscribers != null) {
      if (isPremium) {
        ...
        getSubscribersData(workspaceId, subscribers).then((value) => {
        ...
        let change: any = parseInt(subscribers) / second_to_last;
        setSubscribersCount(subscribers);
        if (subscribers < second_to_last)
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(subscribersCount, 1)}
          </p>
        )}
        {isLoading ? (
