...
async function getWatchTimeData(
  workspaceId: string,
  timeInterval: string,
  watchtime: number
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/watchtimechart`,
      {
        params: {
...
async function getWatchDurationData(
  workspaceId: string,
  timeInterval: string,
  watchduration: any
) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/watchdurationchart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function YoutubeWatchTimeChart({
  workspaceId,
  timeInterval,
  watchtime,
  watchduration,
  ...
  const [selectedFormat, setSelectedFormat] = useState('Watch Time');
  const [watchTimeData, setWatchTimeData] = useState<any>(null);
  const [watchDurationData, setWatchDurationData] =
    useState<any>(null);
  const [screenshot, setScreenshot] = useState(false);

  useEffect(() => {
    if (watchtime != null && watchduration != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Watch Time') {
          getWatchTimeData(workspaceId, timeInterval, watchtime).then(
            (value) => {
              let temp = [];
              ...
              setWatchTimeData(temp);
              setIsLoading(false);
            }
          );
        } else {
          getWatchDurationData(
            workspaceId,
            timeInterval,
            watchduration
          ).then((value) => {
            ...
            setWatchDurationData(temp);
            setIsLoading(false);
          });
        }
      } else {
        if (selectedFormat == 'Watch Time') {
          let value = randomData(timeInterval, watchtime);
          setWatchTimeData(value);
          setIsLoading(false);
        } else {
          let value = randomData(timeInterval, watchduration);
          setWatchDurationData(value);
          setIsLoading(false);
        }
      }
    }
  }, [selectedFormat, timeInterval, watchtime, watchduration]);

  const data = {
    labels:
      selectedFormat == 'Watch Time'
        ? watchTimeData
        : watchDurationData,
    datasets: [
      {
      ...
      <div className="flex flex-row justify-between items-between">
        ...
        <div className="flex flex-row space-x-3">
          ...
          <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() => {
                setSelectedFormat('Watch Duration');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Watch Duration'
                  ? 'bg-white shadow text-gray-800'
                  : 'text-gray-500')
              }
            >
