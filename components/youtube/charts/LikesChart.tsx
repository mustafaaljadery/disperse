...
async function getLikes(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/likeschart`,
      {
        params: {
...
async function getDislikes(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/dislikeschart`,
      {
        params: {
          workspaceId: workspaceId,
          timeInterval: timeInterval,
...
export function YoutubeLikesChart({
  workspaceId,
  timeInterval,
  likes,
  dislikes,
  ...
  const [selectedFormat, setSelectedFormat] = useState('Likes');
  const [likesData, setLikesData] = useState<any>(null);
  const [dislikesData, setDislikesData] = useState<any>(null);
  ...
  useEffect(() => {
    if (likes != null && dislikes != null) {
      if (isPremium) {
        setIsLoading(true);
        if (selectedFormat == 'Likes') {
          getLikes(workspaceId, timeInterval, likes).then((value) => {
            let temp = [];
          ...
          getDislikes(workspaceId, timeInterval, dislikes).then(
            (value) => {
              let temp = [];
              for (let i = 0; i < value.length; i++) {
                if (value[i] < 0) {
        ...
        if (selectedFormat == 'Likes') {
          let value = randomData(timeInterval, likes);
          setLikesData(value);
          setIsLoading(false);
        } else {
          let value = randomData(timeInterval, dislikes);
          setDislikesData(value);
          setIsLoading(false);
        }
      }
    }
  }, [likes, dislikes, timeInterval, selectedFormat]);

  const data = {
    labels: selectedFormat == 'Likes' ? likesData : dislikesData,
    datasets: [
      {
        label: selectedFormat == 'Likes' ? 'Likes' : 'Dislikes',
        data: selectedFormat == 'Likes' ? likesData : dislikesData,
          ...
          <div className="flex flex-row space-x-2">
            {isLoading ? (
              ...
              <p className="text-3xl md:text-4xl text-gray-700 silka-semibold">
                {formatLargeNumber(
                  selectedFormat == 'Likes' ? likes : dislikes,
                  1
                )}
              </p>
            )}
        ...
        <div className="flex flex-row space-x-3 h-full">
          ...
          >
            ...
            <DownloadChart
              ...
              chartData={
                selectedFormat == 'Likes' ? likesData : dislikesData
              }
              labelName={
                selectedFormat == 'Likes' ? 'Likes' : 'Dislikes'
              }
          ...
          <div className="my-auto flex flex-row space-x-1 md:space-x-2 px-1 md:px-2 py-1 md:py-1.5 bg-gray-50 rounded-lg">
            ...
            <button
              onClick={() => {
                setSelectedFormat('Dislikes');
              }}
              className={
                'px-4 md:px-5 py-0.5 md:py-1 rounded text-[11px] md:text-xs silka-medium ' +
                (selectedFormat == 'Dislikes'
                  ? 'bg-white shadow text-gray-800'
                  : 'text-gray-500')
              }
            >
