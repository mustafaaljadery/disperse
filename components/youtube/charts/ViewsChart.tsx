...
async function getViewsData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/viewschart`,
      {
        params: {
...
export function YoutubeViewsChart({
  workspaceId,
  timeInterval,
  views,
  isPremium,
  ...
  useEffect(() => {
    if (views != null) {
      if (isPremium) {
        ...
        let value = randomData(timeInterval, views);
        setTotalViewsData(value);
        setIsLoading(false);
      }
    }
  }, [views, timeInterval]);
        ...
        <div>
          ...
          >
            ...
            <DownloadChart
              ...
              chartName={'Youtube Views'}
              chartValue={views}
              chartData={totalViewsData}
              labelName={'Views'}
              timeInterval={timeInterval}
              chartType={'bar'}
            />
