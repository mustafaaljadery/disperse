...
async function getLikesData(workspaceId: string, likes: number) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}youtube/read/likes`, {
      params: { workspaceId: workspaceId, likes: likes },
    });
    return result.data;
  } catch (e) {
    console.log(e);
...
export function YoutubeLikesAnalytic({
  workspaceId,
  likes,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (likes != null) {
      if (isPremium) {
        setLikesCount(likes);
        getLikesData(workspaceId, likes).then((value) => {
        ...
        let change: any = parseInt(likes) / second_to_last;
        setLikesCount(likes);
        if (likes < second_to_last) change = -Math.abs(1 - change);
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(likesCount, 1)}
          </p>
        )}
        {isLoading ? (
