...
async function getViewsData(workspaceId: string, views: number) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}youtube/read/views`, {
      params: {
...
export function YoutubeViewsAnalytic({
  workspaceId,
  views,
  isPremium,
}: Props) {
  ...
  useEffect(() => {
    if (views != null) {
      if (isPremium) {
        setViewsCount(views);
        getViewsData(workspaceId, views).then((value) => {
        ...
        let change: any = views / second_to_last;
        setViewsCount(views);
        if (views < second_to_last) change = -Math.abs(1 - change);
      ...
      <div className="flex flex-row space-x-3 lg:space-x-1.5 xl:space-x-3">
        {isLoading ? (
          ...
          <p className="silka-semibold text-3xl lg:text-2xl xl:text-3xl">
            {formatLargeNumber(viewsCount, 1)}
          </p>
        )}
        {isLoading ? (
