...
async function getImpressions(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}youtube/read/impressionschart`,
      {
        params: {
...
export function YoutubeImpressionsChart({
  workspaceId,
  timeInterval,
  impressions,
  isPremium,
  ...
  useEffect(() => {
    if (impressions != null) {
      if (isPremium) {
        ...
        let value = randomData(timeInterval, impressions);
        setImpressionsData(value);
        setIsLoading(false);
      }
    }
  }, [timeInterval, impressions]);
