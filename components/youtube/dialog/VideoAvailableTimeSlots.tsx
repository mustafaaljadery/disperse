...
async function createScheduledVideo(
  ...
  try {
    ...
    toast.remove();
    toast.error('Error scheduling vidoe, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
    console.log(e);
  }
  ...
  return (
    <div className="h-10 px-3 w-full flex flex-row justify-between items-between border rounded-lg">
      ...
      <button
        ...
        className="text-[#FF0000] silka-medium text-sm hover:opacity-80"
      >
        Select Time
      </button>
    </div>
...
async function getAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}youtube/read/availableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
async function getMoreAvailableSlots(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}youtube/read/moreavailableslots`,
      {
        params: {
          workspaceId: workspaceId,
          userId: userId,
...
export function YoutubeVideoAvailableTimeSlots({
  workspaceId,
  userId,
  draftId,
  isOpen,
