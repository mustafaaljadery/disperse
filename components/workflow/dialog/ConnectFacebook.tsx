...
interface Props {
  ...
  setFacebookSelectAccountOpen: Dispatch<SetStateAction<boolean>>;
}
...
async function connectFacebook(workspaceId: string) {
  try {
    axiosRetry(axios, { retries: 3 });

    const result = await axios.get(`${apiUrl()}facebook/auth`, {
...
async function checkConnection(workspaceId: string) {
  try {
    ...
    const result = await axios.get(`${apiUrl()}facebook/auth/check`, {
      params: {
...
export function AutomationConnectFacebook({
  isOpen,
  setIsOpen,
  workspaceId,
  setRefetchConnected,
      ...
      const testvalue = setInterval(() => {
        checkConnection(workspaceId).then((value) => {
          if (value.connected == true) {
            ...
            setFacebookSelectAccountOpen(true);
          }
        });
      }, 1000);
    }
          ...
          >
            <div className="flex flex-col w-full justify-start items-start">
              ...
              <p className="mt-2 w-[90%] text-start text-xs silka-regular text-gray-400">
                Follow the instructions below to connect your Facebook
                to Disperse.
              </p>
            </div>
            <div className="flex flex-col mt-6">
              ...
              >
                <div className="flex flex-row space-x-2">
                  <p className="silka-semibold text-[11px] text-[#D90100]">
                    IMPORTANT: Must be a Facebook Page, follow the
                    guide below
                  </p>
                  <svg
                    width="12"
                  ...
                  <div className="flex flex-row space-x-3">
                    ...
                    <div className="my-auto flex flex-col justify-start items-start space-y-0.5">
                      ...
                      <span className="text-[11px] silka-regular text-gray-400">
                        Follow the guide for the exact Facebook setup.
                      </span>
                    </div>
                  </div>
                  <svg
              ...
              <button
                onClick={(e) => {
                  e.preventDefault();
                  setConnecting(true);
                  connectFacebook(workspaceId);
                }}
                disabled={connected}
                className={
                  'py-2 w-full mt-4 bg-[#0572E7] text-sm text-white silka-semibold rounded ' +
              ...
              {connecting && (
                <div className="mt-4 flex flex-row space-x-2 justify-center items-center">
                  <div className="flex flex-row space-x-3">
                    <span className="animate-ping h-2.5 w-2.5 my-auto rounded-full bg-[#0572E7] opacity-75"></span>
