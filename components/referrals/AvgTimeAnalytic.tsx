...
export function AvgTimeAnalytic({
  workspaceId,
  userId,
  timeInterval,
  avgTime,
  ...
  useEffect(() => {
    if (userId && avgTime != null) {
      getData(userId, timeInterval, avgTime).then((data) => {
        setTotal(data[data.length - 1]);
        setAvgData(data);
        setIsLoading(false);
      });
    }
  }, [timeInterval, userId, avgTime]);

  const data = {
    labels: avgData,
    datasets: [
      {
        label: 'Avg Time Sec(s)',
        data: avgData,
        ...
        <div className="flex flex-col space-y-1">
          ...
          {isLoading ? (
            ...
            <p className="text-2xl md:text-4xl text-gray-700 silka-semibold">
              {total == 0 ? '0 Secs' : formatTime(total)}
            </p>
          )}
        </div>
      </div>
