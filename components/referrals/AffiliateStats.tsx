...
async function getPaidConvData(
  userId: string,
  timeInterval: string,
  paidConv: number
) {
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}referrals/paidconvchart`,
      {
        params: {
          userId: userId,
...
async function getSignupsData(
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}referrals/signedupchart`,
      {
        params: {
          userId: userId,
          timeInterval: timeInterval,
...
async function getConversionsData(
  userId: string,
  timeInterval: string,
  conversionRate: number
) {
  ...
  try {
    const result = await axios.get(
      `${apiUrl()}referrals/conversionratechart`,
      {
        params: {
          userId: userId,
          timeInterval: timeInterval,
...
function Component({
  ...
  return (
    <div className="w-full md:w-1/3 flex flex-col p-2 md:p-5">
...
export function AffiliateStats({
  ...
  // total
  const [totalPaidConv, setTotalPaidConv] = useState(0);
  const [totalSignups, setTotalSignups] = useState(0);
  const [totalConversions, setTotalConversions] = useState(0);

  // data
  const [paidConvData, setPaidConvData] = useState([]);
  const [signupsData, setSignupsData] = useState([]);
  const [conversionsData, setConversionsData] = useState([]);

  //loading
  const [paidConvLoading, setPaidConvLoading] = useState(true);
  ...
  useEffect(() => {
    if (
      ...
      getPaidConvData(userId, timeInterval, paidConv).then((data) => {
        setPaidConvData(data);
        setTotalPaidConv(data[data.length - 1]);
        setPaidConvLoading(false);
      });
      getSignupsData(userId, timeInterval, signups).then((data) => {
        setSignupsData(data);
        setTotalSignups(data[data.length - 1]);
        setSignupsLoading(false);
      });
      getConversionsData(userId, timeInterval, conversionRate).then(
        (data) => {
          setConversionsData(data);
          setTotalConversions(data[data.length - 1]);
          setConversionsLoading(false);
        }
      );
    }
  }, [timeInterval, userId, paidConv, signups, conversionRate]);

  const mapData = [
    {
      title: 'Paid Conversions',
      data: paidConvData,
      loading: paidConvLoading,
      total: totalPaidConv,
    },
    {
      title: 'Signups',
