...
export function PaidConversionsChart({
  workspaceId,
  userId,
  timeInterval,
  conversions,
  ...
  useEffect(() => {
    if (userId && conversions != null) {
      getData(workspaceId, timeInterval, conversions).then((data) => {
        setTotal(formatLargeNumber(data[data.length - 1], 1));
        setPaidData(data);
        setIsLoading(false);
      });
    }
  }, [timeInterval, userId, conversions]);
  ...
  return (
    <div className="w-full flex flex-col p-2 md:p-5">
