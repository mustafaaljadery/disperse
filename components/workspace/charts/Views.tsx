...
async function getViewsData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}workspace/read/contentviews`,
      {
        params: {
...
export function ViewsChart({
  ...
  useEffect(() => {
    if (workspaceId) {
      setIsLoading(true);
      getViewsData(workspaceId, timeInterval).then((value) => {
  ...
  return (
    <div className="mt-4 px-6 border rounded py-6">
      <div className="flex flex-col space-y-1">
        <h2 className="text-base silka-semibold">Content Views</h2>
        <p className="text-xs silka-regular text-gray-500">
          Views on content posted using Disperse
        </p>
      </div>
      <div className="mt-8">
        {isLoading ? (
