...
async function getFollowersData(
  ...
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}workspace/read/followerschart`,
      {
        params: {
...
export function FollowersChart({
  ...
  useEffect(() => {
    if (workspaceId) {
      setIsLoading(true);
      getFollowersData(workspaceId, timeInterval).then((value) => {
  ...
  return (
    <div className="mt-4 px-6 border rounded py-6">
      <div className="flex flex-col space-y-1">
        <h2 className="text-base silka-semibold">Followers</h2>
        <p className="text-xs silka-regular text-gray-500">
          Gained followers in time period
        </p>
      </div>
      <div className="mt-8">
        {isLoading ? (
