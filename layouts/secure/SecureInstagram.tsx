...
import { InstagramNotConnected } from '../notconnected/InstagramNotConnected';
...
async function getConnected(workspaceId: string) {
  try {
    ...
    const result = await axios.get(
      `${apiUrl()}instagram/read/isconnected`,
...
interface SecureInstagramProps {
  loading: boolean;
...
export function SecureInstagram({
  loading,
  setLoading,
  children,
  setWorkspaceId,
  setIsPremium,
}: SecureInstagramProps) {
