...
async function downgrade(workspaceId: string) {
  try {
    ...
    toast.loading('Downgrading to Pro...', {
      className: 'text-sm silka-medium text-gray-900',
      duration: 80000,
    });
    const result = await axios.post(
      `${apiUrl()}workpace/update/downgradetopro`,
      null,
      {
        params: {
    ...
    if (result.data.message == 'success') {
      ...
      toast.remove();
      toast.error('Error downgrading to pro, please try again.', {
        className: 'text-sm silka-medium text-gray-900',
      });
    }
    return result.data;
...
export function DowngradeToProDialog({
  isOpen,
  setIsOpen,
  workspaceId,
}: Props) {
