...
async function mergeVideos(
  ...
  files.forEach((file: any) => {
    filesTemp.push(file?.videoUrl ? file?.videoUrl : file?.url);
  });
  try {
    const result = await axios.post(
      `${apiUrl()}editor/create/mergevideos`,
      null,
      {
        params: {
          workspaceId: workspaceId,
          projectId: projectId,
          files: filesTemp.toString(),
          size: size,
        },
      }
    );
...
export default function MergeVideos() {
  ...
  return (
    <PageHead title="Merge Videos · Disperse Video Tools">
      <ToolsLayout workspaceId={workspaceId} title="Merge Videos">
        <div className="flex flex-row h-full w-full divide-x">
          <div className="w-full h-full flex-1 space-y-8 flex flex-col justify-center items-center p-6">
            {file ? (
              <div className="h-full w-full px-4 flex flex-col justify-center items-center">
                <video
                  loop
                  src={file?.videoUrl ? file?.videoUrl : file?.url}
                  className="p-2.5 w-full rounded h-[80vh] bg-black overflow-hidden"
                  controls
                  autoPlay
                ></video>
              </div>
            ...
            <div className="flex flex-row w-full justify-start items-start space-x-3">
              {files.length == 0 ? (
                <>
                  <button className="p-3.5 hover:border-[#FF623d] text-[10px] silka-regular text-gray-400 h-[80px] w-[80px] border rounded-lg border-dashed">
                    Select a video
                  </button>
                </>
              ) : (
                <>
                  {files.map((value: any, index: number) => {
                    return (
                      <button
                        ...
                        key={index}
                        className={
                          'h-[80px] w-[80px] rounded-lg hover:opacity-80 border border-dashed ' +
                          (value?.id == file?.id
                            ? 'border-[#FF623D]'
                            : '')
                        }
                  ...
                  <button
                    ...
                    className="p-3.5 hover:border-[#FF623d] text-[10px] silka-regular text-gray-400 h-[80px] w-[80px] border rounded-lg border-dashed"
                  >
                    Select a video
                  </button>
                </>
              ...
              <>
                <div className="flex flex-col">
                  ...
                  {file ? (
                    <div className="mt-6 flex flex-col">
                      <h2 className="silka-semibold text-[#363636] text-sm">
                        Selected Video
                      </h2>
                      <video
                        src={
                        ...
                        className="mt-2 w-full max-h-[200px] bg-black rounded-lg"
                        controls
                      ></video>
                  ...
                  <div className="mt-6 flex flex-col">
                    <h2 className="text-sm silka-semibold text-gray-900">
                      Total Time
                    </h2>
                    <p className="mt-1.5 text-base silka-medium text-gray-500">
                      {files
                        .map((value: any) => {
                  ...
                  <div className="mt-6 flex flex-col">
                    ...
                    <DropdownMenuPrimitive.Root>
                      <DropdownMenuPrimitive.Trigger asChild>
                        <button className="flex mt-3 flex-row justify-between items-between py-2 px-3 rounded-lg border">
                                ...
                                return (
                                  <DropdownMenuPrimitive.Item className="w-full">
                                    <button
                                      ...
                                      className="text-sm py-2 w-full hover:bg-gray-50 rounded silka-medium text-[#363636]"
                                    >
                                      {value}
                                    </button>
                ...
                >
                  <DialogPrimitive.Trigger asChild>
                    <button
                      className={
                        'py-2 w-full text0sm silka-semibold text-white bg-[#Ff623D] rounded ' +
                        (files.length == 0 ? 'hover:opacity-90' : '')
                      }
                    >
                      Export
                    </button>
