...
function encodeTime(seconds: number) {
  // encodee to 00:00:00:00
...
async function trimVideo(
  ...
  try {
    const result = await axios.post(
      `${apiUrl()}editor/create/trimvideo`,
      null,
      {
        params: {
    ...
    console.log(e);
    toast.remove();
    toast.error('Error rendering video, please try again.', {
      className: 'text-sm silka-medium text-gray-900',
    });
  }
}
...
export default function TrimVideo() {
  const [workspaceId, setWorkspaceeId] = useState('');
  ...
  const timeFunction = () => {
    ...
    if (Number(videoRef?.current?.currentTime) < Number(start)) {
      videoRef.current.currentTime = String(start);
      videoRef?.current?.play();
    }
    if (Number(videoRef?.current?.currentTime) >= Number(end)) {
      videoRef.current.currentTime = String(start);
      videoRef?.current?.play();
    }
  };
  ...
  useEffect(() => {
    if (videoRef && videoRef?.current) {
      console.log('start', start, 'nd', end);
      videoRef?.current?.addEventListener('timeupdate', timeFunction);
      return () => {
  ...
  }, [videoRef, videoRef?.current, start, end]);

  useEffect(() => {
    const time = encodeTime(start);
    setStartStr(time);
  }, [start]);

  useEffect(() => {
    const time = encodeTime(end);
    setEndStr(time);
  }, [end]);

  useEffect(() => {
    if (router.isReady) {
      setWorkspaceeId(router.query.workspaceId as string);
    }
  }, [router.isReady]);
  ...
  return (
    <PageHead title="Trim Video · Disperse Video Tools">
      <ToolsLayout workspaceId={workspaceId} title="Trim Video">
            ...
            {!file ? (
              ...
              <div className="w-full h-full px-4 flex flex-col justify-center items-center">
                <video
                  ...
                  ref={videoRef}
                />
                <div className="px-16 mt-5 flex flex-col justify-center items-center space-y-4 w-full">
                  <p className="text-sm silka-semibold">
                    {(end - start).toFixed(1)} secs
                  </p>
            ...
            {renderingStatus == 'None' ? (
              <>
                <div className="flex flex-col w-full">
                  ...
                  <div className="mt-6 flex flex-row space-x-5 w-full">
                    <div className="flex w-1/2 flex-col space-y-2">
                      <h2 className="text-gray-700 text-xs silka-regular">
                        Start time
                      </h2>
                      <input
                        disabled={!file}
                        ...
                        className="w-full rounded text-sm border silka-regular border-gray-300 focus:ring-0 focus:border-[#FF623D]"
                        type="text"
                      />
                    </div>
                    <div className="flex w-1/2 flex-col space-y-2">
                  ...
                  <div className="mt-6 flex flex-col space-y-2">
                    ...
                    <DropdownMenuPrimitive.Root>
                      <DropdownMenuPrimitive.Trigger asChild>
                        <button
                          disabled={!file}
                          className="flex flex-row justify-between items-between border border-gray-300 rounded-lg py-2 px-3"
                        >
                                ...
                                return (
                                  ...
                                  >
                                    <button
                                      ...
                                      className="py-1.5 rounded w-full hover:bg-gray-50 text-sm silka-medium text-[#363636]"
                                    >
                                      {value}
                                    </button>
                                  </DropdownMenuPrimitive.Item>
                  ...
                  <div className="mt-6 flex flex-row space-x-3"></div>
                </div>
                <DialogPrimitive.Root
                  open={exportOpen}
